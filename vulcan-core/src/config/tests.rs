...
fn tasknotes_skipped_settings_report_unsupported_categories() {
    ...
    assert!(skipped.iter().any(|item| {
        item.reason == "TaskNotes Bases view integration settings are not yet supported"
    }));
}
...
#[test]
fn tasknotes_mdbase_migration_converts_assets_and_disables_legacy_generator() {
    let temp_dir = TempDir::new().expect("temp dir should be created");
    let vault_root = temp_dir.path();
    let plugin_dir = vault_root.join(".obsidian/plugins/tasknotes");
    ...
    fs::write(
        ...
        r#"{"unrelated":{"keep":true},"enableMdbaseSpec":true}"#,
    )
    ...
    fs::write(
        ...
        r"---
name: task
description: Generated by TaskNotes
...
fields:
  ...
  status: {type: enum, required: true, values: [open, done], default: open, tn_role: status, tn_completed_values: [done]}
  dateCreated: {type: datetime, required: true, generated: now, tn_role: dateCreated}
x-tasknotes:
  nlp: {triggers: []}
---

Generated TaskNotes type.
",
    )
    .expect("legacy type");
    let paths = VaultPaths::new(vault_root);
    let raw = serde_json::json!({"enableMdbaseSpec": true});

    let preview = tasknotes_migrate_mdbase_assets(&paths, &raw, true).expect("preview");
    assert_eq!(preview.migrated_files.len(), 3);
    assert!(fs::read_to_string(vault_root.join("mdbase.yaml"))
        .expect("config")
        .contains("0.2.0"));
    assert!(fs::read_to_string(plugin_dir.join("data.json"))
        .expect("settings")
        .contains("\"enableMdbaseSpec\":true"));

    let applied = tasknotes_migrate_mdbase_assets(&paths, &raw, false).expect("apply");
    assert_eq!(applied.migrated_files.len(), 3);
    let plugin = fs::read_to_string(plugin_dir.join("data.json")).expect("settings");
    assert_eq!(
        serde_json::from_str::<Value>(&plugin).expect("valid JSON")["unrelated"]["keep"],
        true
    );
    assert!(plugin.contains("\"enableMdbaseSpec\":false"));
    assert!(fs::read_to_string(vault_root.join("mdbase.yaml"))
        .expect("config")
        .contains("spec_version: \"0.3.0\""));
    let type_source = fs::read_to_string(vault_root.join("_types/task.md")).expect("type");
    assert!(type_source.contains("kind: mdbase.type"));
    assert!(type_source.contains("x-tasknotes:"));

    let collection = crate::mdbase::load_mdbase_collection(vault_root)
        .expect("collection should load")
        .expect("collection should exist");
    let registry = crate::mdbase::load_mdbase_type_registry(&collection).expect("type registry");
