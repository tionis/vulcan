...
Knowledge bases remain Vulcan's focus. Registered directories have a versioned, device-local `knowledge` or `files-only` capability profile. `knowledge` remains the backward-compatible default; `files-only` lets the daemon manage a full Git working tree without indexing Markdown or running knowledge services. Both profiles currently use full-tree materialization. Selective materialization, partial clone, shallow-history handling, and selective LFS transfer remain planned in [12.20](docs/ROADMAP.md#1220-selective-materialization-and-large-repository-synchronization).
...
Further selective-materialization work is tracked in [12.20](docs/ROADMAP.md#1220-selective-materialization-and-large-repository-synchronization): sparse checkouts, selective Git LFS downloads, partial clones, and separately scoped shallow-history handling. These capabilities are not advertised for sync. The design separates a directory's logical file tree from the subset available on each device; excluded files must never become accidental deletions. Managing a development checkout or retaining a complete backup requires an explicit policy beyond synchronizing its working files.
