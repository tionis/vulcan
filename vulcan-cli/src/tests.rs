...
fn init_git_repo(vault_root: &Path) {
    ...
    run_git(vault_root, &["config", "core.autocrlf", "false"]);
}
    ...
    assert!(matches!(
        ...
        Command::Sync {
            command: SyncCommand::SemanticPlan {
                ...
                ref from,
                ref to,
                group_by: SemanticGroupingArg::Change,
                agent: true,
...
#[test]
fn parses_supplied_file_sync_resolution() {
    let cli = Cli::try_parse_from([
        "vulcan",
        "sync",
        "resolve",
    ...
    .expect("supplied-file resolution should parse");
    assert!(matches!(
        cli.command,
        Command::Sync {
            command: SyncCommand::Resolve {
    ...
    assert_eq!(
        ...
        Command::Refactor {
            command: RefactorCommand::SplitNote {
                ...
                keep_source: true,
                preserve_missing_fragments: true,
                no_navigation: true,
                dry_run: true,
                no_commit: true,
            },
