...
multi-vault consent selects exactly one wiki for each grant. `remote set --add-wiki <id>` adds a
registered wiki, `--wiki <id>` selects one for policy edits, and `--remove-wiki <id>` revokes that
wiki's grants before removal. Stop the instance before changing its definition. The definition
contains no token secret and is not copied when the vault is synchronized.
...
The `knowledge` or `files-only` profile belongs to the device-local daemon registration, not the vault configuration documented below. Set it with `vulcan vault add <id> <path> --profile files-only`, `vulcan vault clone <remote> <path> --profile files-only`, or `vulcan vault set <id> --profile files-only`; omit the option or choose `knowledge` to retain the established knowledge-vault behavior. `vulcan vault add <id> <path> --no-sync` registers a local directory without enabling Git sync. The CLI spelling `files-only` is stored as `files_only` in the device registry's TOML/JSON. Files-only status and mutation JSON includes versioned `capabilities`; the object is omitted for knowledge registrations to preserve existing JSON shapes. `materialization = full` is the only supported device-local materialization policy, independent of either content profile; selective materialization remains a later sync feature. `vulcan sync doctor [<wiki>]` uses a registered profile, or `--profile files-only` for a direct path. See the [Git sync guide](../guide/git-sync.md#managed-directory-profiles) for current boundaries.
...
- `publish.outline.profiles.<name>.query_json` — type: `string`; target: `shared|local`
  ...
  Preferred command: `vulcan config set`
  Example: `vulcan config set publish.outline.profiles.wiki.base_url '"https://outline.example.com"'`
- `publish.outline.profiles.<name>.remove_toc` — type: `boolean`; target: `shared|local`; default: `false`
  One-way Outline target, query, credential environment binding, and request limits.
  Preferred command: `vulcan config set`
  Example: `vulcan config set publish.outline.profiles.wiki.base_url '"https://outline.example.com"'`
