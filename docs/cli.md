...
- `vulcan publish outline <profile> [--dry-run] [--create-collection] [--adopt-pulled] [--interactive] [--overwrite-conflict <source-path>] [--overwrite-conflicts]`: reconcile a configured publication into Outline with human-readable phase/item progress. When `collection_id` is absent, `--create-collection` (or profile `auto_create_collection = true`) creates the configured `collection_title`, persists its UUID in shared config, and continues publication. `--adopt-pulled` explicitly reuses selected durable pull bindings only when the remote content, title, parent, and collection still match the last successful pull; ambiguous ownership fails closed. Remote drift stops publication unless terminal-guided, selective, or all-conflict overwrite authorization is supplied; see [Outline publishing](guide/outline-publishing.md).
- `vulcan outline collections list|show|create|update|archive|restore|bind ...`: discover accessible Outline collections and UUIDs, create and automatically bind a collection to a profile, update collection metadata/access defaults, recoverably archive or restore a collection, or bind an existing UUID. Every mutating command supports `--dry-run`; archiving a still-bound collection requires `--allow-bound`, and Vulcan does not expose permanent collection deletion.
- `vulcan pull outline <profile> --into <vault-directory> [--dry-run] [--root-document <remote-id>]... [--max-depth <depth>] [--exclude-document <remote-id>]... [--max-documents <count>] [--max-content-bytes <bytes>] [--max-attachments <count>] [--max-attachment-bytes <bytes>] [--max-total-attachment-bytes <bytes>] [--interactive|--overwrite-conflicts|--conflict-markers] [--apply-remote-moves] [--archive-missing <vault-directory>|--delete-missing --confirm-delete-count <count>] [--archive-stale-attachments <vault-directory>|--delete-stale-attachments --confirm-stale-attachment-delete-count <count>] [--no-commit]`: pull the profile's explicitly configured collection into a contained local namespace, reverse supported Outline Markdown, materialize referenced attachments under `_attachments/`, and reconcile through durable three-way state. Document count, cumulative Markdown, attachment count, individual attachment size, cumulative attachment downloads, and API response bodies are bounded. Changing, duplicate, or incomplete pagination snapshots fail closed. Repeated roots select the union of their subtrees; depth zero selects only each root, while exclusions remove a named document and all descendants. Local/remote drift fails closed unless the user chooses remote overwrite or Git-backed diff3 merging; non-overlapping edits are applied automatically and only overlapping hunks receive localized markers. Live mutation holds the shared vault write lock and journals an `operation_id` before every action; interrupted runs report `resumed_operation` and reuse that journal until the final scan succeeds. Human output reports pull phases and items on stderr; `--quiet` suppresses progress. Remote title and hierarchy changes keep stable local paths unless `--apply-remote-moves` is explicitly selected. Missing remote documents and stale managed attachments are retained unless their respective archive/delete policies are explicitly selected; active documents outside the current selector are reported as `out_of_scope`, never as missing.
- `vulcan integration list|show|validate|plan|run|status`: inspect, validate, dry-run, execute, and diagnose named `[integrations.routes.<name>]` Outline routes. `integration run <name>` composes pull/push according to `direction` and `authority`; `--interactive` reviews `review` conflicts, `--all` runs every enabled route, and `--scheduled` runs only routes whose `@hourly`, `@daily`, `@weekly`, or `every N[m|h|d]` interval is due. Live runs are locked and checkpointed outside the cache.
- `vulcan integration bindings|bind|unbind <route> ...`: list durable identities, adopt an existing local Markdown file as an existing immutable Outline document ID without rewriting either side, or remove only that identity state. Exact `document_bindings` in route config override generated pull paths. See [Outline publishing](guide/outline-publishing.md).
...
PDF converters commonly emit explicit page anchors such as `<span id="page-135-0"></span>`. Vulcan preserves those anchors and retargets links to the generated note that owns them, while excluding the raw tag from generated note titles and paths. Rewritten Markdown destinations include a `.md` extension when a local fragment becomes cross-note and use CommonMark angle brackets when spaces or parentheses require them.

The source note is replaced by default. `--keep-source` retains it as an archival copy while creating the tree at a distinct folder-note path. Use `--no-navigation` to suppress generated parent/child lists. Missing source fragments fail closed by default. For noisy conversions with links to page IDs that never existed, `--preserve-missing-fragments` retains those already-broken links against the generated root (or the retained source) and emits one warning per fragment; it does not claim to repair them. Markdown footnotes and reference-style definitions currently fail closed because their definitions are file-local; convert them to inline links or keep the affected material within one output note before splitting.
...
vulcan --output json artifact import ./book.mdaf \
  --destination Rules/Book --from-level 2 --through-level 3 --dry-run
vulcan artifact import ./book.mdaf \
  --destination Rules/Book --hierarchy outline \
  --from-level 2 --through-level 3
```

Import always requires a new explicit vault-relative destination. Hierarchy authority defaults to `auto`: a validated aligned `outline.json` is preferred when it can represent the requested projection, with a diagnosed Markdown-heading fallback when no usable outline is available. Use `--hierarchy markdown` or `--hierarchy outline` to force either authority; forced outline requires a completely aligned `outline.json` and fails instead of falling back. Reports identify the authority actually used. The default level range starts at level 2 and includes level 3; when `--from-level` is changed without `--through-level`, the importer includes one descendant level, capped at level 6. The dry-run report lists every generated note, copied asset, source span, diagnostic, and rewritten link without creating the destination. A live import streams declared assets, adds `vulcan.source` artifact identity and source locators to each note, rewrites declared asset links and uniquely resolvable normalized source references, refreshes the cache, and removes the partial destination if writing or reindexing fails. The MDAF package remains external evidence rather than becoming canonical vault state.
