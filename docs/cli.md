...
- `vulcan publish outline <profile> [--dry-run] [--adopt-pulled] [--interactive] [--overwrite-conflict <source-path>] [--overwrite-conflicts]`: reconcile a configured publication into an existing Outline collection with human-readable phase/item progress. `--adopt-pulled` explicitly reuses selected durable pull bindings only when the remote content, title, parent, and collection still match the last successful pull; ambiguous ownership fails closed. Remote drift stops publication unless terminal-guided, selective, or all-conflict overwrite authorization is supplied; see [Outline publishing](guide/outline-publishing.md).
