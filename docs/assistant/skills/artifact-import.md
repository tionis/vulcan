...
- Plan with `vulcan --output json artifact import <artifact> --destination <new-folder> --dry-run`. The default `auto` authority prefers a validated aligned outline that can represent the requested projection and otherwise uses Markdown with an `outline_fallback` diagnostic; the report identifies the authority actually used. Defaults materialize levels 2–3, or one descendant level when `--from-level` is supplied alone. Force `--hierarchy markdown|outline` and explicit level bounds when the desired projection differs; forced outline fails instead of falling back.
...
- Compare fresh-destination previews using each note's `source_bytes`, title and parent path, plus `review.diagnostic_counts` and `review.largest_note_source_bytes`. `review.source_coverage_complete` checks exact, nonoverlapping primary-byte ownership; it does not establish semantic correctness. A `large_note_source` warning identifies notes over 50,000 source bytes (excluding generated navigation/frontmatter). Review those notes individually; do not blindly split them by size. `review.semantic_review_required` remains true even with clean diagnostics and `doctor` results.
...
- Plain-text source references may become links only when their authored placement and destination note are unique. Repeated labels use their source occurrence order only when safe plain-text source/output placements correspond exactly. Coarse source ranges spanning several notes and protected syntax remain unchanged with diagnostics; do not force them to the first matching note. Repeated heading fragments resolve only within a unique nearest owning-note subtree; unresolved ambiguity remains visible.
...
- Check the generated vault with `vulcan doctor`. In shortest-resolution mode, explicit `./` and `../` links resolve against their source directory without filename/alias fallback. A missing explicit path needs repair, not a same-named substitute; valid same-named notes elsewhere must not make it ambiguous.
