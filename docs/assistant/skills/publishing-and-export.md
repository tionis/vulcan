...
- Prefer a named `[integrations.routes.<name>]` route for an Outline wiki that will be pulled or pushed repeatedly. Start with `vulcan integration validate <name>` and `vulcan integration plan <name>`, then use `integration run <name>`. Validation parses the referenced outgoing query, query JSON, or additive selection plan so malformed selectors fail before remote planning. The route owns local/remote subtree topology, direction, authority, archival policy, exact ID/path overrides, work limits, and interval; the referenced publication profile still owns API connection, outgoing selection, and rendering transforms.
...
- On Android, route checkpoints and Outline pull/publication mappings use per-vault Termux-private state. An older shared-storage state file is copied on the first live operation while the private lock is held; do not delete the old state or downgrade to a build that writes it again during migration.
...
- Check each conflict's `conflict_markers_available` value. Missing managed notes and attachment conflicts cannot use text markers; retain/cancel them or explicitly overwrite from Outline. A missing managed note is never silently reported unchanged.
- Manage written diff3 markers with `--conflict-operation status|continue|abort`. Status and abort are local and need no token; preview abort with `--dry-run`. Continue fails while any complete or malformed managed marker remains, and abort restores only the LOCAL side after preflighting every file and permission.
...
- Review `remote_capabilities` in pull JSON: Vulcan validates the Markdown response contract, collection membership, acyclic hierarchy, and consistent revision/timestamp coverage. A live apply re-lists the collection and fails before mutation unless the ID-sorted snapshot is identical; Outline has no documented stable server-semver probe, so conformance is behavioral.
- Keep `--max-content-bytes`, `--max-attachments`, `--max-attachment-bytes`, and `--max-total-attachment-bytes` proportional to the reviewed collection. Raise a limit only after checking the dry-run scope; API response bodies retain an independent hard cap.
- Portable sibling collisions, including Unicode-collision cases, are deterministically disambiguated with remote-ID-derived suffixes. Treat remaining path-length or missing-parent errors as remote hierarchy issues to resolve in Outline; do not hand-edit durable mappings. Reference-style and complex Markdown attachment/document destinations remain supported and code spans are ignored. Review `diagnostics` for preserved HTML, unsupported directives, or attachment destinations that could not be rewritten safely.
...
- Outline mappings under `.vulcan/publish/outline/` on desktop and the corresponding per-vault private directory on Android are durable state, not rebuildable cache. Do not delete or hand-edit them as a routine repair step.
- Do not replace a profile collection UUID when durable publication state exists. Collection archive is recoverable but requires `--allow-bound` while any profile still references the UUID; Vulcan intentionally does not expose permanent collection deletion.
...
- List an Outline workspace's collections, create and bind a missing player wiki, then publish the configured selection into it.
