...
- When the record is large, add `--path-offset <n> --path-limit <1-256>` and follow the explicit `next_offset`; use progress totals to distinguish pending, in-flight, applied, and rebase-required groups. Do not diagnose absent paths from a single page.
