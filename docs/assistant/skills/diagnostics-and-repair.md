...
6. Inspect `state.recovered_from` and `state.retained` in JSON sync output. Recovery journals are authoritative device-local operational state outside `.vulcan/cache.db`; do not remove them as a cache repair.
