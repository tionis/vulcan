...
6. For missing background updates, run `vulcan daemon status`. It authenticates a live loopback capability request and reports the runtime address, PID, uptime, and registrations; a stale runtime record is reported as stopped. Restart with `vulcan daemon start --detach` only after confirming no service is live, and use `vulcan daemon stop` for graceful watcher/worker shutdown. Direct `vulcan sync run` remains a valid diagnostic and recovery path without the daemon.
