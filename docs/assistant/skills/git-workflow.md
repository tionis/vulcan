...
- A local companion may expose conflict proposal creation only when its daemon has a server-configured resolver. Companion requests select bounded context and broad-read opt-in but never supply provider endpoints or credentials; capability negotiation is authoritative. Proposal approval and rejection use the same retained IDs, previews, permission checks, stale-input validation, and audit transactions as the direct CLI.
