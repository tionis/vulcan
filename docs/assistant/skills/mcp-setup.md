...
3. Start it with `vulcan mcp remote run <name>`, or use `vulcan daemon start --detach` to host configured remotes as resident listeners. Give the client the single public MCP URL printed by init/show. Proxy the MCP path, OAuth metadata paths, and `/oauth/*` to that loopback listener.
...
- Restart the daemon after `remote init`, `set`, or `remove` to reload resident listeners. Do not start the same remote with `remote run` while the daemon owns it; the instance lock rejects overlap. The listener can route multi-vault definitions, but the current `remote init`/`set` CLI still configures one vault per remote; use separate named remotes until the multi-vault management CLI is added.
