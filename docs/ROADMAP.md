...
- [x] Add deterministic sync performance gates that measure top-level Git process work instead of flaky wall-clock timing. An unchanged cycle is limited to one remote query, no redundant fetch, one atomic accepted-ref transaction, one stable two-pass worktree capture, and at most 35 Git subprocesses; stuck Git/filter/transport process groups have a tested configurable timeout.
