...
- [x] Add deterministic sync performance gates that measure top-level Git process work instead of flaky wall-clock timing. An unchanged cycle uses the persistent private index to bypass full capture, is limited to one remote query, no redundant fetch or accepted-ref rewrite, cached-stat verification, cached filter-path analysis with exact attribute-source invalidation, batched repository discovery, requirement-environment inspection, and hidden-ref reads, and at most 25 Git subprocesses even with a fresh Git LFS readiness probe; changed worktrees retain the stable two-pass capture and changed accepted refs retain one atomic transaction. Stuck Git/filter/transport process groups have a tested configurable timeout, and direct interactive progress is transient by default with durable diagnostics behind `--verbose`.
...
- [x] Add a machine-local rolling-release signing handoff that independently validates the successful exact-commit release, canonical manifest, five archives, hashes, sizes, version, channel, and unsigned payload before signing and replacing only `vulcan-update-channel.json`. Keep the raw private key out of GitHub-hosted runners and fail visibly rather than publishing a falsely trusted descriptor.
