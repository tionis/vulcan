...
**First slice shipped:** Device-local `knowledge` (the backward-compatible default) and `files-only` registration profiles, with a versioned capability projection and profile-aware direct/daemon sync. Files-only sync skips Markdown index refresh and knowledge-specific workers/validation; concurrent automatic merges remain for review so local profile choice cannot silently accept different bytes. Selective materialization and the remaining registry, development-checkout, backup, and conformance work remain open.
...
- [ ] Define versioned capability/config/report contracts for managed directories. Shipped device-local `files-only` and `knowledge` profiles over shared sync machinery, with compatibility-preserving omitted knowledge profiles. Keep verifying migration and rollback for existing registrations and reject every incompatible combination before mutation.
- [ ] Separate directory observation/scheduling from index and content services. Shipped registered files-only sync and daemon observer gating without implicit Markdown indexing. Extend coverage across direct status/restart/conflicts/recovery and index-free operation on arbitrary directories. Keep reusable finite workflows in `vulcan-app`, file mechanics in dependency-light `vulcan-sync`, and content semantics in `vulcan-core`.
- [ ] Gate knowledge endpoints and workers by explicit capabilities with actionable diagnostics. Files-only sync skips indexing, knowledge validation, scripts, semantic history, and agent resolution. Extend gates to every endpoint/worker and test they do not activate based on extension. Preserve deterministic shared merge policy across different local profiles; local capability choices may reduce automation, never silently select different accepted bytes.
