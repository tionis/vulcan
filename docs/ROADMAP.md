...
- [ ] Host MCP HTTP through the daemon transport layer. Preserve authentication/OAuth behavior and capability negotiation; scope sessions, tool-pack selection, notifications, and cancellation to their original client/vault authority rather than global daemon state.
  - The daemon now accepts host-provided ingress services under the same supervised readiness, status, and shutdown graph. Its CLI host registers one aggregate listener service for configured named MCP remotes; each one-vault remote uses the foreground OAuth/MCP listener and per-instance lock with independent session state. Listener readiness is reported only after bind and context setup; daemon shutdown stops its accept loop and closes active MCP sessions/SSE streams. The host constructs one scheduler for ingress adapters and its executor. Resident MCP request workers acquire read/write vault permits, revalidate their original OAuth/grant authority after queuing, and retain permits until the worker finishes even if the caller times out. Named sessions also apply profile narrowing without widening. Cross-process write locking, durable unknown-write recovery, multi-vault routing, and full resident/foreground conformance remain open.
...
- [ ] Host the same definitions through 10.7 without changing OAuth, MCP schema, or consent
  ...
  - Resident startup now supervises configured one-vault remotes as an aggregate ingress service, reusing the same named-remote OAuth/MCP listener and instance locks as foreground `remote run`. It closes active sessions/SSE streams on shutdown and rejects multi-vault definitions rather than silently narrowing them. Resident request workers use the host's per-vault scheduler with post-queue authority revalidation and timeout-safe permit ownership. IndieAuth profile and metadata discovery now have bounded requests/responses, no automatic redirects, and a combined network deadline within the resident startup budget; token exchange is bounded separately. Cross-process write locking, durable unknown-write recovery, and multi-vault routing still gate completion.
