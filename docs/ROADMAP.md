...
- [x] Add one clearly labelled rolling nightly or development prerelease only when `main` advanced since the last published development build. Prefer a single scheduled eligibility check plus manual dispatch, reuse the canonical package builders, and replace or prune the previous rolling release instead of accumulating permanent nightly tags and assets. Publish it under the distinct force-moved `rolling-main` tag so fetches never collide with the `main` branch.
