...
**Depends on:** The implemented 12.5 structured merge and conflict-preservation boundary. Individual format adapters depend on their owning format contract: Phase 19.13.3 for replicated-store changesets/event sets and Phase 14/16 only if an Automerge document becomes a synchronized canonical artifact rather than device-local crash-recovery state.
...
- [ ] Specify the shared replicated-store changeset/event envelope needed by 12.19: immutable record identity, actor/sequence and checkpoint lineage, schema version, payload digest/signature fields, canonical packed-set ordering when packing is necessary, duplicate semantics, and explicit collision/gap/compaction failures. Prefer one immutable record per Git path so most concurrent additions need no content driver.
