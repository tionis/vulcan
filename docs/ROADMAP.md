...
- [ ] Move note CRUD orchestration out of `vulcan-cli`: note create/append/set planning, writes, plugin hook dispatch, and shared diagnostics/check passes now live in `vulcan-app`, but patch/delete and scan refresh logic still need extraction
