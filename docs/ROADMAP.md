...
- [x] Add deterministic sync performance gates that measure top-level Git process work instead of flaky wall-clock timing. An unchanged cycle is limited to one remote query, no redundant fetch, one atomic accepted-ref transaction, one stable two-pass worktree capture, cached-stat verification, cached filter-path analysis with exact attribute-source invalidation, and at most 34 Git subprocesses even with a fresh Git LFS readiness probe; stuck Git/filter/transport process groups have a tested configurable timeout, and direct interactive progress is transient by default with durable diagnostics behind `--verbose`.
