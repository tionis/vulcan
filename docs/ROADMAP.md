...
- [ ] Host the same definitions through 10.7 without changing OAuth, MCP schema, or consent
  ...
  - The durable hosted-operation ledger now records non-secret caller principal, grant credential ID, and audience alongside service instance and canonical vault. Scoped status matching rejects other callers and older unbound records. The shared hosted executor can pre-register a caller operation before a response worker starts, then dispatch it once only for the same authority. MCP timeout dispatch and the authenticated status surface still need to be wired to this ledger.
