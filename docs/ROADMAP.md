...
- [ ] Run ordinary Git three-way merge and configured attribute drivers first, followed by deterministic structured mergers for supported Markdown/frontmatter, JSON, Canvas, Bases, and selected Obsidian/plugin state formats.
  - [x] Dispatch unresolved Git paths through bounded deterministic Markdown-frontmatter, recursive JSON, JSON Canvas keyed-object, and Bases YAML mergers. All conflicted paths must resolve under policy before Vulcan writes an exact alternate-index merge tree; parse failures and ambiguous/delete-modify inputs remain preserved conflicts. Selected Obsidian/plugin state formats remain future policy work.
