...
- [x] Keep direct archives and `cargo install --locked --path vulcan-cli` documented as development/fallback paths. Consider native DEB/RPM, AUR, or other repository packages only after demand warrants their ongoing update and signing obligations; do not block the first supported desktop release on every distribution ecosystem.
