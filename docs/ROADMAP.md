...
- [ ] Materialize deterministic conflict copies when needed so non-conflicting paths can continue synchronizing, while marking affected paths unresolved. Define whether human-visible artifacts live beside notes or under a configurable managed directory and ensure Obsidian indexing behavior is explicit.
  - [x] Build and retain a typed conflict-materialization candidate for bounded blob conflicts. The candidate keeps the already accepted remote object at each original conflicted path, puts the local object under `.sync-conflicts/<conflict-id>/local/<original-path>`, includes ordinary clean merge results, fails closed on structural objects or managed-root collisions, and is anchored by the conflict-record commit without changing the worktree or remote. The hidden root is synchronized but excluded from Vulcan/Obsidian note indexing. Publishing, application, cleanup, and resolution-lease migration remain before the parent item is complete.
