...
- [x] Pull the checked-out branch from its upstream inside each finite cycle before the hidden live refs move, following the repository's own pull configuration with `--no-edit` and no implicit autostash. Divergence past `pull.ff=only`, interactive rebase requests, and merge/rebase conflicts pause only the branch lane with a reason; dirty worktrees defer to the next cycle; repositories without an upstream, detached HEADs, and bare repositories skip. The branch is never pushed.
