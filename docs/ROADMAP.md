...
- [x] Bound keyword-search candidate memory with 128-row filtering batches, including regex and section-scoped searches. Preserve SQL ranking and explanation ranks, continue past rejected batches, and stop once the requested number of surviving hits is available. Regression tests verify bounded consumption and limited/unlimited result equivalence across sorts. Bundled vault-query skill review: query syntax and result contracts are unchanged.
...
- [x] Inventory all listeners, watcher owners, periodic loops, child processes, credentials, durable state, and shutdown paths, including MCP stdio/HTTP, `serve`, site/bundle previews, conflict/semantic workers, notifications, auto-commit, and implemented app endpoints. Record current command, module, configuration, intended owner, migration slice, and compatibility tests in a checked-in design record. Distinguish implemented functionality from planned Phase 11/19 capabilities. See `docs/specs/unified-hosting.md`.
- [x] Define resident hosting from device configuration and temporary foreground hosting from invocation-local configuration. Temporary `serve`/preview hosts must not persist registration, install services, or enable unrelated automation. Preserve existing single-vault routes and flags through adapters or an explicit migration contract.
- [x] Define service IDs, global/vault/instance scope, dependency order, readiness, health, cancellation, restart policy, and configuration validation. Device config owns host services/listeners, identity, credentials, and execution trust ceilings; canonical vault config owns application schedules, enabled state, and target nodes under 10.8. Specify restart-required versus live-reconciled settings; invalid execution updates block affected scheduled dispatch rather than silently retaining stale automation.
- [x] Specify direct, explicit daemon, and automatic client routing before adding flags. Keep ordinary direct operations daemon-independent. Automatic fallback requires a known pre-dispatch failure; ambiguous writes require operation-status lookup or an explicit indeterminate result. Do not silently start a resident daemon.
- [x] Define ownership when temporary hosts overlap a resident host: daemon-attached sessions reuse its runtime; standalone hosts acquire the applicable ownership locks or report an actionable conflict. No competing automatic mutation workers for the same canonical vault/repository.
- [x] Reconcile overlapping Phase 10 checkboxes against code and tests without marking new consolidation work complete merely because a sync-specific equivalent exists. Document preserved JSON/protocol contracts and any versioned migration.
...
- [x] Move `vulcan-cli/src/serve.rs` transport handling to reusable daemon axum routers over existing app workflows. Compose companion and available vault APIs; keep adding the remaining 10.3 endpoints as separate feature items rather than requiring the entire REST backlog for this migration.
...
- [ ] Extract remaining MCP command dispatch dependencies on CLI handlers/options into shared app workflows and transport-neutral protocol types. Choose a shared protocol module/crate in the 10.7.1 record if needed; neither daemon nor app may import CLI. Preserve tool names, schemas, report/error shapes, resources, prompts, pagination, and permission filtering.
  - Request argument types and protocol constants live in `vulcan-app::mcp_protocol`; tool JSON Schemas live in `vulcan-app::mcp_schemas`; the built-in tool catalog, pack selection, annotations, and permission visibility live in `vulcan-app::mcp_catalog`. `vulcan-app::mcp_dispatch` owns JSON-RPC request framing behind a method-handler boundary; `vulcan-app::mcp_assistant` owns permission-filtered prompt/skill discovery, prompt rendering, resource discovery/templates, vault-owned assistant reads, and pack-filtered custom-tool resource reads; `vulcan-app::mcp_help` owns built-in help topics, report types, and help-resource response shaping, with command-specific help injected by the host; `vulcan-app::mcp_completion` owns permission-filtered completion and response shaping with the host's help-topic catalog injected. `vulcan-app::mcp_read_tools` owns search/query workflows, the note-source read boundary, guarded daily-content and bounded daily-list response shaping, and permission-filtered task-query reports; `vulcan-app::periodic` owns shared date/target resolution and daily list/show reports, used directly by both CLI and MCP. MCP task list/query and create/complete/reschedule now call app task workflows directly, retaining the existing write preflight and cache refresh. MCP status uses the existing `vulcan-app::browse` report directly. `vulcan-app::notes` resolves vault/direct Markdown targets and owns `note_outline` and `note_get` reports for CLI and MCP. Both current transports use these shared contracts. Remaining tool workflows, CLI-derived command-help catalog sharing, and HTTP/OAuth routing still need extraction.
  - `vulcan-app::web` now owns permission-checked web search/fetch entrypoints shared by CLI and MCP, including network checks and save-path/write preflight. MCP web tools no longer import CLI web handlers or option enums; feature-disabled builds retain explicit errors. Live MCP search/fetch and denied-network/save-path regressions cover this boundary.
  - `vulcan-core::graph` now owns the note-link confidence query previously embedded as CLI SQL, and `vulcan-app::notes` owns the `note_info` report shared by CLI and MCP. MCP applies its read filter to backlink and confidence counts, so unreadable sources cannot alter reported totals. CLI/MCP JSON parity, scoped-read behavior, and installed agent guidance are covered.
  - MCP `note_delete` now invokes `vulcan-app::notes` directly, refreshes the cache after application, and filters backlink previews through the current read guard before reporting counts or paths. A restricted-profile dry-run regression covers the scoped preview.
  - MCP `note_set` now invokes the app mutation and shared checked-report builder directly. CLI/MCP report parity and checked/unchecked diagnostic tests cover the preserved response contract.
  - MCP `note_create` and `note_append` now invoke app mutations and shared checked-report builders directly, retaining template/frontmatter/variable parsing, path preflight, cache refresh, and auto-commit. CLI/MCP JSON parity and literal MCP argument tests cover this boundary.
  - MCP `note_patch` now resolves targets, applies scoped edits, and builds checked/dry-run reports through `vulcan-app::notes` without CLI handler/options imports. Scoped CLI/MCP JSON parity and dry-run diagnostic tests cover the retained contract.
- [ ] Host MCP HTTP through the daemon transport layer. Preserve authentication/OAuth behavior and capability negotiation; scope sessions, tool-pack selection, notifications, and cancellation to their original client/vault authority rather than global daemon state.
  - The daemon now accepts host-provided ingress services under the same supervised readiness, status, and shutdown graph. Its CLI host registers one aggregate listener service for configured named MCP remotes; each remote uses the foreground OAuth/MCP listener and per-instance lock with independent session state. Listener readiness is reported only after bind and context setup; daemon shutdown stops its accept loop and closes active MCP sessions/SSE streams. The host constructs one scheduler for ingress adapters and its executor. Resident MCP request workers acquire read/write vault permits, revalidate their original OAuth/grant authority after queuing, and retain permits until the worker finishes even if the caller times out. Named sessions also apply profile narrowing without widening. Consent and session initialization select a grant-bound vault from multi-vault definitions; the CLI adds, updates, and removes those per-vault ceilings while the instance is stopped. Cross-process write locking, durable unknown-write recovery, and full resident/foreground conformance remain open.
...
### 10.8 Vault-owned cron schedules and execution trust

**Goal:** Run finite workflows such as RSS-to-Markdown ingestion on explicitly selected nodes, with synced/live-reloaded configuration and optional admin-signed execution. Keep the same model for one local vault and a Git-synchronized home-server/laptop setup.

**Design contract:** [Vault-owned schedules and execution trust](specs/scheduled-execution.md). This is planned work, not a claim of shipped commands. No coordinators, distributed claims/leases, automatic failover, or universal job-queue rewrite are required.

**Depends on:** 10.7.2–10.7.4 host lifecycle, observation, execution context, and mutation coordination. App targets additionally use 19.10; finite built-in targets do not depend on the full App platform. Stable node selection reuses existing device IDs and 12.15 migration compatibility. Static-signature verification needs public keys and Git only, not private-key custody, the full Phase 17 identity system, or future sigchains.

- [x] Record vault-owned assignment, two execution trust policies, immutable execution snapshots, portable RSS bindings, eventual reassignment/revocation limits, and the extension path to existing key registries. Review bundled configuration/sync guidance; design-only changes do not advertise unimplemented commands.
- [ ] Freeze the canonical schedule schema and vault-visible path with App settings: stable job ID, typed target/input, package/config identity, service authority, enabled state, exact target node IDs, trigger, and bounded limits. No duplicate local job-enablement flag; daemon startup/registration, identity, secrets, and trust ceilings remain local. Multiple selected nodes explicitly mean independent execution.
- [ ] Implement one supervised bounded scheduler with anchored intervals and documented five-field cron/IANA timezone semantics. Default to coalesced missed runs, no overlap, and at most one pending follow-up. Define DST, suspend/resume, clock jumps, retry limits, cancellation, and shutdown using an injected clock; defer one-shot and elaborate catch-up policies until needed.
- [ ] Persist occurrence identity and replay-safe dispatch outside `cache.db`, retaining deduplication watermarks independently of prunable history. Reuse domain job ledgers and ordinary cross-process mutation locks; replay interrupted operations only when explicitly idempotent. Do not claim exactly-once effects.
- [ ] Reconcile complete execution snapshots on configuration changes. Disable/reassign stops new claims and requests cooperative cancellation; existing invocations retain immutable code/config. Block affected work on malformed, conflicting, or untrusted changes, with bounded diagnostics. An offline former runner can continue until it receives the change; document this rather than adding leases.
- [ ] Add one shared synchronous execution-authorizer boundary for Apps, scheduled built-ins, and applicable direct invocations. Repository trust accepts incoming changes within existing sandbox/permission ceilings; optional trusted-commit mode verifies native Git SSH signatures against protected static `allowed_signers` plus exact principal/namespace/vault/purpose constraints. Vault changes cannot disable enforcement, replace roots, or expand device permissions.
- [ ] Bind execution to the full approved definition/dependency snapshot: package digests, entrypoints, execution settings, capture rules, and requests. Verify exact Git objects and validated package bytes without candidate hooks/filters/verifiers. Ordinary wiki data remains live; unsigned data-only commits may proceed only while the execution definition matches accepted authority. Unsigned execution edits/disablement cannot fall back to an old job. Define source-ref selection, merge approval, ancestry, rollback/fork checks, policy generations, first bootstrap, and lost/restored accepted-state handling before implementation.
- [ ] Keep package signatures, execution approval, registry administration, and automatic device/sync signatures distinct. Static execution keys ship first; Phase 12.17.6 supplies scoped registry-backed authority later without replacing scheduler types. Require a deliberate role contract rather than silently reusing registry-admin keys that currently have exclusive usage.
- [ ] Expose planned schedule list/show/edit/disable/run/next/history workflows with JSON and mutation-free previews; show target/local node identity, active/pending definition, trust decision, next local/UTC times, last success/error, and cancellation. Exact CLI names are frozen with the schema. Scheduling requires a daemon; direct finite operations remain available, with applicable trust checks and no implicit daemon startup.
- [ ] Document local/non-Git, one designated server plus Git-sync, static admin keys, configuration reload, offline reassignment, and repair. Existing specialized sync/update schedulers keep their contracts; migrate only explicitly inventoried applicable automation with compatibility tests, not by incidental rewrite.
- [ ] Test crash windows, duplicate dispatch, history pruning, DST/resume, reload/cancel races, direct-versus-daemon writes, two offline runners, unsigned dependency substitution, signed merges, known rollback, key removal, and permission changes. Update relevant bundled skills and installed-payload tests when behavior ships.
...
**Depends on:** Existing registry/direct-sync workflows and 12.1's synchronous boundary. Integrate with 10.7's shared runtime as it lands without requiring the full App platform. Full-materialization files-only management can ship before 12.20; selective materialization requires its conformance gates. Design contract: `docs/design_document.md` section 4.7.

**Shipped:** Device-local `knowledge` (the backward-compatible default) and `files-only` registration profiles, versioned capability reporting, full-tree materialization, optional Git sync registration, and profile-aware direct/daemon sync and doctor. Files-only operation skips Markdown indexing and knowledge services; concurrent automatic merges remain for review so local profile choice cannot silently accept different bytes. Unattended files-only sync pauses on unsupported repository states and does not claim safety against concurrent external Git activity. File replication is not a complete backup. Selective materialization remains under 12.20.

- [x] Record the accepted scope, layering, implementation order, and current-versus-planned distinction in the design document, README, and sync guide. Review existing bundled sync/Git skills and the AGENTS template for affected guidance.
- [x] Define versioned capability/config/report contracts for managed directories. Shipped device-local `files-only` and `knowledge` profiles over shared sync machinery, with compatibility-preserving omitted knowledge profiles. Verify migration and rollback for existing registrations and reject incompatible combinations before mutation.
- [x] Separate directory observation/scheduling from index and content services. Registered files-only sync and daemon observers operate without implicit Markdown indexing; direct status, conflicts, recovery, and restart remain available without a note cache. Keep reusable finite workflows in `vulcan-app`, file mechanics in dependency-light `vulcan-sync`, and content semantics in `vulcan-core`.
- [x] Gate knowledge endpoints and workers by explicit capabilities with actionable diagnostics. Files-only sync skips indexing, knowledge validation, scripts, semantic history, and agent resolution. Preserve deterministic shared merge policy across different local profiles; local capability choices may reduce automation, never silently select different accepted bytes.
- [x] Keep materialization policy device-local and independent of content profile. Registration, clone, status, doctor, and direct paths support the full-tree policy; selective materialization remains in 12.20. Registration alone does not initialize Git, contact a remote, enable sync when `--no-sync` is selected, or trust repository execution.
- [x] Define an explicit active-development-repository support contract: unattended files-only sync rejects staged changes, multiple worktrees, detached HEAD, in-progress Git operations, and nested repositories/submodules, and rechecks the selected branch before application. External Git concurrency remains outside Vulcan's lock, so actively changing development checkouts are unsupported. Preserve existing knowledge-vault behavior.
- [x] Distinguish file replication from repository backup. Metadata convergence and a full working-tree copy are not a complete backup; a later backup profile needs explicit ref/history/LFS coverage, retention, and restore tests. Defer general Git hosting/IDE management and standalone product extraction.
- [x] Test defaults/overrides and registry migration, mixed knowledge/files-only registrations, direct and daemon profile handling, no-index operation, knowledge workflow compatibility, normal-index preservation, and unsupported-operation diagnostics. CLI JSON and daemon contract tests cover no implicit index or script activation.
- [x] Update README shipped/planned claims, `docs/guide/git-sync.md`, `docs/reference/config.md`, CLI/API/help/discovery/completions, and installation/companion guidance where affected. Extend existing `sync-workflow` and `git-workflow` skills and review `docs/assistant/AGENTS.template.md`. Validate installed skill payloads and meaningful command availability.
...
- [ ] Host the same definitions through 10.7 without changing OAuth, MCP schema, or consent
  ...
  - Resident startup now supervises configured remotes as an aggregate ingress service, reusing the same named-remote OAuth/MCP listener and instance locks as foreground `remote run`. It closes active sessions/SSE streams on shutdown. Resident request workers use the host's per-vault scheduler with post-queue authority revalidation and timeout-safe permit ownership. IndieAuth profile and metadata discovery now have bounded requests/responses, no automatic redirects, and a combined network deadline within the resident startup budget; token exchange is bounded separately. Multi-vault definitions require explicit vault selection at consent, and the selected grant vault controls session initialization and request policy. The CLI now adds, updates, and removes per-vault ceilings through `remote set` while an instance is stopped; removing one vault revokes only its grants and refresh families. Ordinary `note create`, `note set`, `note append`, vault-relative `note patch`, and `note delete` now take the cross-process vault lock for their filesystem mutation; replacements and deletion reject stale content after plugin hooks, and creates refuse concurrent collisions. Managed mdbase writes retain their journaled lock boundary. Other write paths (including template side effects), durable unknown-write recovery, and full resident/foreground conformance still gate completion.
  - The durable hosted-operation ledger now records non-secret caller principal, grant credential ID, and audience alongside service instance and canonical vault. Scoped status matching rejects other callers and older unbound records. Named foreground and resident listeners share a per-instance executor that pre-registers mutations before the request worker starts, records outcomes, and recovers interrupted operations after taking ownership. A timed-out mutation returns its operation ID and an authenticated, caller-bound status path; its MCP session is retired so stale in-memory state cannot be reused. Read requests remain off the durable ledger. Other write paths, bounded ledger retention, and full resident/foreground conformance still gate completion.
...
- [x] Implement `GitCliEngine` first and make it the only supported engine for the initial release. One selected engine owns every mutating operation in a repository cycle; do not interleave CLI and embedded-library writers against the same repository transaction. All commands share one captured-I/O spawn boundary; on Windows it applies `CREATE_NO_WINDOW` so scheduled daemon sync does not flash a terminal for each Git process or helper.
...
- [x] Support both colocated `.git` repositories (default for ordinary Linux/Windows/local use) and detached Git directories with a materialized worktree. Record the latter only in device-local registry/state.
  - [x] Repair Git dubious-ownership discovery failures after a successful clone by adding only the canonical new worktree to global `safe.directory` and retrying. Cover colocated and detached checkouts with real Git ownership checks; preserve unrelated trust entries and ordinary discovery protections. Dry runs do not mutate trust configuration.
...
- [x] Keep sync job journals and other device-local operational state under the platform state directory, Git directories under the platform data directory, and credentials in Git/SSH credential facilities. Never store credentials or pending sync state in the rebuildable cache. The planned 12.15 device key is the explicit interim exception: it uses a protected Vulcan user-data identity store until later SSH-agent/keychain support, never vault or cache storage.
  ...
  - [x] Read version-1 sync journals and apply markers as recovery witnesses after an upgrade; keep the Android vault write lock and rebuildable SQLite WAL cache in per-vault Termux state, outside the shared worktree. Leave an older shared cache untouched and rebuild it in private state with `vulcan scan`.
  - [x] Put Android mdbase recovery journals, staging, receipts, and outbox plus route, Outline pull, and Outline publication state and locks under per-vault Termux state. Copy legacy state under the private lock before mutation, preserving the old files. Probe canonical mdbase directory sync before opening a new write transaction and fail before mutation when the shared filesystem rejects it.
  - [ ] Certify these workflows on real Android shared storage, including migration from interrupted mdbase writes and Outline source snapshots, canonical file replacement and directory `fsync`, and downgrade behavior. A filesystem that cannot sync canonical directories remains unsupported for mdbase writes until a separately reviewed recovery contract exists.
...
- [x] `vulcan vault clone <remote> <path> [--id <id>] [--git-dir <path>]` — clone and register one wiki, supporting a detached Git directory for constrained filesystems; `--dry-run` reports the worktree, Git directory, redacted remote, platform policy, and proposed registration without mutation. Clone uses the typed Git CLI engine and preserves a successfully cloned worktree if the subsequent device-local registration fails.
  - [x] Add `vulcan sync clone <remote> <path>` as the sync-oriented convenience surface. It preserves explicit layout overrides, derives the wiki ID, selects native defaults on desktop, and automatically uses `android-shared` with a detached Git directory under the private Vulcan data directory in Termux. Missing private parent directories are planned without dry-run mutation and created only when the clone is applied.
...
- [x] Enforce the invariant that all current local bytes are reachable from a Git ref before applying a remote tree. Recheck the captured tree before application and recapture when files change during a synchronization attempt.
  - [x] Publish every mutating cycle's captured worktree to one exact-lease remote per-device safety head before pull or canonical reconciliation. Bridge divergent reuse of a device identity without losing either history; provide `sync devices list`, explicit fetch into durable local recovery refs with live comparison, and fail-closed pruning limited to fetched, fully integrated device backup heads while retaining the local recovery anchor.
  - [x] Show optional shared friendly names alongside full IDs in `sync devices list`; provide `set-name` and `clear-name` with dry runs while keeping ref and recovery identity unchanged.
  - [x] Make `sync devices list` a read-only inventory of remote safety heads, local recovery freshness, retained recovery copies, and named devices without backups. Keep exact refs and revisions visible without implying a fetched copy is safe to remove.
  - [x] Keep local recovery refs, shared labels, and the current local ID visible when remote observation fails; distinguish an unavailable remote from an observed absence and redact raw Git failure text from the inventory report.
  - [x] Name the destructive action `sync devices prune-backup`, retain `remove` as a compatibility alias, and state in its plan and completion output that it does not revoke device, Git, or vault access.
...
- [x] Pause reconciliation and working-tree application during merge/rebase/cherry-pick/bisect operations or unexplained HEAD commit/ref movement. A finite cycle first captures the current bytes under the local candidate ref and fetches an existing remote live tip, then retains a device-local paused journal with a structured exact reason. Staged normal-index changes sync as ordinary worktree bytes through the alternate index instead of pausing; the cycle never resets the normal index or silently switches branches.
- [x] Pull the checked-out branch from its upstream inside each finite cycle before the hidden live refs move, following the repository's own pull configuration with `--no-edit` and no implicit autostash. Divergence past `pull.ff=only`, interactive rebase requests, and merge/rebase conflicts pause only the branch lane with a reason; dirty worktrees defer to the next cycle; repositories without an upstream, detached HEADs, and bare repositories skip. After a successful file lane, publish the branch tip with the observed tracking ref as an exact lease, never force-pushing; moved remotes and transport or policy failures record a detail without failing the converged file lane.
...
- [x] Run ordinary Git three-way merge and configured attribute drivers first, followed by deterministic structured mergers for supported Markdown/frontmatter, JSON, Canvas, Bases, and selected Obsidian/plugin state formats.
  ...
  - [x] Retain successful per-path structured resolutions when sibling paths still require review. The validated provisional tree carries deterministic results forward, substitutes accepted-remote bytes only for the remaining projectable paths, and preserves a conflict record scoped to those unresolved paths; structural projections continue to fail closed.
  - [x] Seed explicit resolution indexes from Git's complete merge-tree candidate before replacing reviewed or structured conflict paths. This preserves clean content merges outside the conflict set instead of leaving their three index stages unresolved; regression tests cover both supplied-content and preserved-side resolution while proving the normal index remains untouched.
...
- [x] Give every conflict an immutable ID derived from merge base, candidate tips, paths, and policy version/hash. Preserve base/local/remote objects and all conflicting file contents before publishing or applying a conflict-preserving result. The Git backend creates per-conflict base/local/remote refs before returning, while `vulcan-app` atomically persists an immutable device-local record plus content-addressed base/local/remote artifacts outside the vault and rebuildable cache.
  - [x] Make monolithic conflict-record reads and writes obey the same bounded compatibility ceiling, while retaining access to records above the former 1 MiB read-only ceiling. Regression tests prove oversized writes fail before creating an unreadable record. Paged conflict-session storage supersedes this compatibility format in the conflict-scale follow-on.
  - [x] Store new conflict path metadata in bounded, content-verified pages behind a small durable manifest while retaining transparent reads for version-1/2 and monolithic compatibility records. Pages split on serialized byte size as well as path count, aggregate digests stream without a second complete path buffer, and explicit path-count/diagnostics ceilings reject impossible metadata before allocation. Manifest publication occurs only after every page is durable; incomplete or tampered page sets fail closed, and a 10,000-path regression proves round-trip behavior without a monolithic record.
  - [x] Assign versioned deterministic resolution-group identities while persisting conflict paths. Ordinary blob conflicts remain independent singleton groups, structural/synthesized paths are conservatively atomic, whole-tree failures retain an explicit whole-tree group, and legacy records are normalized on read. Group identity is invariant under path arrival order and candidate-role swaps.
  - [x] Keep mutable group progress outside immutable conflict evidence in bounded per-batch records. Each batch binds its canonical group selection and digest, expected accepted revision, method identity, exact tree/commit, recovery witness, and prepared/published/applied/needs-rebase state; overlapping active selections fail closed while stale batches remain auditable and can be replanned.
  - [x] Resolve bounded group selections by preserved side against the current accepted tree. `sync resolve --group <id> --side ...` overlays only complete selected groups, validates the exact resulting whole tree, publishes with a current-frontier lease plus batch-specific refs/trailers, resumes through durable batch witnesses, and leaves every unselected group actionable. Unrelated accepted-live advancement is carried forward when selected provisional objects are unchanged; changed group inputs fail for fresh reconciliation.
  - [x] Route selection-scoped reviewed-file proposals through the same durable group-batch transaction. Version-4 proposals bind complete group IDs, their digest, and the exact accepted frontier; approval verifies exact selected bytes/modes and forbids out-of-selection changes, never silently replans reviewed output after frontier movement, and leaves other groups pending. Rejected and terminal proposals remain audit history without blocking a new proposal.
  - [x] Extend selection-scoped review to patch and editor workflows without changing patch semantics. Patches still apply to the immutable original local candidate and extract only complete selected-group blobs; editor plans materialize only selected paths. Both carry the captured selection/frontier through the handoff, so later proposal creation fails stale instead of rebinding reviewed bytes to a newer accepted tree.
  - [x] Preserve independent unfinished obligations when a later sync creates another conflict. Replacement supersession is allowed only when its path set covers every pending or rebase-required path from the older session; clean sync retains grouped sessions regardless of legacy record version, so unrelated formatter or editor activity cannot silently retire pending choices.
  - [x] Reconcile interrupted group batches after accepted-live advancement. An unpublished prepared batch moves durably to `needs_rebase` and is replanned on the new frontier when its selected inputs are unchanged; a published batch whose commit is an ancestor of current live applies that descendant so intervening accepted changes survive. Resume verifies that every recorded resolution commit still names its recorded tree.
  - [x] Expose explicit conflict progress and bounded detail responses across direct CLI and companion HTTP. Summaries report path/group/pending counts, detail reports group states, `--path-offset` plus `--path-limit` reads and returns only overlapping evidence pages with at most 256 paths and total/next-page metadata, and paged progress marks its returned group subset explicitly while retaining global counters. Companion capabilities advertise the identical pagination and group-batch ceilings so callers never infer truncation or partial-selection support; legacy records use a safe compatibility scan.
  - [x] Bound conflict-list work and output independently of each session's path count. Current paged sessions read only the first 16-path preview plus indexed counters; summaries expose `paths_returned` and `paths_complete`, and human output marks truncated previews rather than implying a complete path list.
  - [x] Batch conflict tree inventories, blob reads, blob writes, and alternate-index overlay updates so Git subprocess growth is bounded independently of conflicted-path count. Reuse the bulk object boundary for structured merging, classification, projection, exact-tree verification, and durable artifact metadata; replace quadratic classification lookup with a path map. A 200-path process-count regression requires two Git processes for selected reads and one for blob writes.
  - [x] Mark conservative text/Markdown formatting candidates in conflict classification without changing `require_review` or selecting a winner. The hint compares whitespace-normalized immutable sides and fails closed for code fences, hard breaks, indented code, and HTML-shaped lines; formatter execution remains an explicit reviewed proposal boundary.
  - [x] Batch agent proposal input reads across immutable base/local/remote sides and enforce the aggregate request budget while assembling each file. A 200-path regression requires exactly three Git tree inventories rather than per-path object subprocesses; provider execution remains outside the repository lock.
- [x] Project a deterministic provisional tree when needed so non-conflicting paths can continue synchronizing while affected paths remain explicitly unresolved outside the vault.
  - [x] Build, publish, and apply a typed conflict projection for bounded blob conflicts. The accepted remote object remains provisionally at each original conflicted path, ordinary clean merge results continue to every device, and no conflict artifacts enter the canonical vault. The reproducible conflict-record commit anchors this exact tree, is published under `refs/heads/__vulcan-sync/conflicts/<id>/record`, and advances the live ref with compare-and-swap before Git-native worktree application; reports and durable records distinguish published/applied state. Structural objects, stale remote leases, and worktree drift fail closed. Reviewed resolution publishes a sibling remote resolution ref while retaining the immutable original base/local/remote inputs and conflict refs.
...
- [x] Treat an LLM/agent as an explicit escalation after deterministic merging, not as a merge driver whose output is assumed deterministic or correct.
  ...
  - [x] Add repeatable complete-group selection to direct and companion agent proposal creation. Selected requests permission-check and disclose only selected eligible paths, include the immutable group/frontier binding in provider input and proposal identity, enforce the byte budget over that subset, and apply through the shared proposal batch transaction. Unselected binary or internal conflicts neither leak to nor disqualify the selected request.
  - [x] Negotiate scoped companion agent proposals separately from side-resolution group batches. Capabilities advertise the exact proposal request version, selected requests require that version, and unknown or future selection fields fail closed instead of degrading to whole-conflict disclosure.
  - [x] Add an explicit formatter-assisted proposal path for conservative formatting-candidate groups. It pins the executable bytes, exact reported version, arguments, optional config bytes, and accepted frontier; normalizes private base/local/remote copies under byte and process-time ceilings; requires a clean normalized three-way merge; and retains the result for ordinary review without auto-accepting it.
...
- [x] Make the default agent operation produce a `ResolutionProposal` containing a patch/tree, explanation, referenced context, input conflict ID, model/provider identity, prompt/tool-contract version, and validation results. It must not write directly to the worktree or refs.
  ...
  - [x] Extend proposal format version 4 with an optional immutable conflict-group selection/frontier binding while preserving version-3 identity verification for retained proposal records.
- [x] Add preview, explicit approval, stale-input detection, cancellation, redacted audit logging, and deterministic revalidation before applying a proposal. Automatic acceptance is a separate per-policy opt-in and still obeys all path, parse, link, deletion, and final-tree checks.
  ...
  - [x] Keep reviewed complete-file resolution distinct from agent eligibility. `sync resolve --file` accepts exact reviewed bytes for internal Obsidian state and binary conflicts without provider disclosure, and can populate Git-synthesized rename destinations with a regular-file mode; LLM proposals continue to reject any conflict containing those inputs. Regression coverage exercises both the CLI path and synthesized-mode boundary.
  ...
  - [x] Add an opt-in daemon conflict worker for one designated resolver node. Device-local configuration requires an explicit wiki allowlist, bounded group and polling limits, the daemon resolution provider, and each vault's separate local `sync.agent_auto_accept` switch. Each pass skips paused or actively syncing repositories, selects only pending singleton regular-file Markdown/text overlaps with complete bounded evidence, disables broad context, shares the companion's per-conflict claim, and reuses the unchanged proposal/approval transaction. Durable content-free status exposes accepted proposal and resolution commits, review-only skips, errors, and restart-preserved provider backoff through `vulcan daemon conflict-status`.
...
- [x] Watch registered worktrees, debounce editor save sequences, impose a maximum dirty age, perform safety rescans after watcher overflow, and reconcile remotes on startup/resume and periodically. The future notification/WebSocket layer only adds another idempotent trigger.
  - [x] Avoid idle CPU saturation from daemon sync watchers hashing the entire worktree every second alongside native notifications. Start content-comparing polling only when native setup fails, at a 30-second interval; retain periodic supervisor reconciliation for missed events and regression coverage for native delivery and forced polling fallback.
  - [x] Eliminate idle supervisor ledger serialization, replacement, and fsync on empty job claims. Wake the worker when jobs are enqueued, become runnable, or shutdown is requested; cover empty, retained, cancelled, and blocked queues plus ordinary and idempotent enqueue wake-ups. Bundled skill review: this internal scheduling change does not alter commands, permissions, or agent workflows.
  ...
  - [x] Harden the shared vault watcher for exhausted, silent, and transiently failing native backends. Failed setup or runtime notification channels fall back to content-comparing polling, transient errors for ignored `.vulcan` files do not terminate polling, and a 30-second incremental safety rescan detects missed changes even when a registered native watcher emits no event. Safety reports carry zero native events and still drive the ordinary scan/rebuild callback.
  - [x] Reduce shared-watcher work: honor the complete quiet-period debounce across shutdown-check slices, separate 30-second content polling from editor debounce, and scan known edited files directly while retaining full discovery for structural changes and rescan signals. Native signals rehash even timestamp-preserving edits. Update the installed index-maintenance skill with freshness expectations.
  - [x] Replace one-millisecond Git child-process polling with completion-driven timeout waits, including the termination grace period. Preserve concurrent output draining, bounded timeouts, and process-group cleanup; cover concurrent completions and timeout termination. Bundled skill review: command and sync safety contracts are unchanged.
  - [x] Replace daemon shutdown polling with wakeable cancellation for job workers, per-wiki watchers, notification listeners, semantic workers, and runtime deadlines. Idle watchers park without timers; file events, backend disconnection, and shutdown all wake them. Regression tests cover queued/blocked jobs, cancellation races, watcher deadlines, and disconnection.
  - [x] Share one event-driven status snapshot producer across connected companion clients. Supervisor transitions and successful companion mutations invalidate it, external changes reconcile every 30 seconds, and disconnecting the last client releases it. Preserve snapshot JSON and direct fresh status reads; update the bundled sync-workflow skill with external-change freshness expectations.
...
- [x] Provide companion operations for capabilities, wiki listing, sync/status/pause/resume, conflict list/detail/resolution proposals, semantic plans, job status/cancellation, and event subscription. Do not expose an unrestricted Git command endpoint.
  ...
  - [x] Add dry-runnable native per-user service installation. `vulcan daemon install/uninstall` atomically manages a restartable `systemd --user` unit on Linux or an XML-defined Windows logon task bound to the current user SID with an interactive-token least-privilege principal, invokes service managers without a shell or elevation, rejects symlinked definition replacement, references only the current absolute executable, keeps optional environment-only provider credentials in a device-local file, and preserves all registrations/state/vault files on uninstall. SID-derived Windows task names prevent cross-user collisions; the task uses detached start and is run immediately after registration instead of waiting for another logon. The Windows 11 24H2+ release keeps one console-subsystem executable and embeds `consoleAllocationPolicy=detached`, so scheduled launches allocate no transient terminal while ordinary terminal calls retain CLI semantics; release CI verifies the embedded manifest. Platform-neutral plan tests cover the renderers and CLI JSON tests prove dry-run creates no service definition.
...
- [x] Build a thin Obsidian companion that requests editor saves, triggers the daemon/direct bridge, displays state and conflicts, and opens reviewed resolutions. The cross-platform plugin ships as a self-contained bundle under `integrations/obsidian-vulcan`, stores bearer authority only through Obsidian 1.11.4+ `SecretStorage`, allowlists non-secret persisted settings, refuses non-loopback endpoints, negotiates protocol v1 and authenticated event snapshots, debounces completed vault writes without racing busy/paused/conflicted state, and uses mandatory dry-run plus a second warning-styled action for preserved-side resolution. Mock-daemon tests cover authenticated status, idempotent sync, conflict detail, preview, and apply. It never invokes Git or implements an independent synchronization state machine.
  - [x] Embed the distributable companion assets in Vulcan and add `vulcan daemon companion install [<wiki>] [--dry-run]`. The reusable installer targets only registered Obsidian vaults, rejects symlinked installation boundaries, atomically replaces each installed or updated managed asset, preserves existing plugin settings and unrelated files, and seeds only the non-secret daemon endpoint/wiki settings on first install. Bearer pairing remains an explicit transfer into Obsidian `SecretStorage`; no token is written into the vault or plugin data.
  - [x] Notify once for each authoritative failed daemon job or retained failed transaction through both authenticated event snapshots and polling recovery. Bound displayed detail, deduplicate by durable identity, provide an opt-out, and keep temporary companion connectivity loss distinct from synchronization failure. Add companion controls to ignore network failures or notify once after a configured failure count or observed duration; expose retained job details in the status dialog.
  - [x] Treat checked-out branch pull or publication failures as failed daemon jobs with `error` status while explicitly retaining that the hidden file lane converged, so aggregates and companion notices cannot present a partially successful operation as wholly healthy.
  - [x] Publish a healthy checked-out branch lane before hidden file reconciliation so ordinary unpushed commits are not stranded by a later preserved file conflict; keep branch fetch/pull available first and prove the lanes remain independent with a real conflict regression.
  - [x] Make `sync status` describe its actual read-only preview semantics: distinguish a completed inspection from successful synchronization, summarize the branch preview, and classify the hidden file lane from observed refs plus worktree/snapshot equality without fetching, merging, applying, or claiming conflict detection.
  - [x] Include durable unresolved conflict records in registered `sync status` so an up-to-date accepted file lane cannot hide conflicts still awaiting review. Report conflict-state read failures as incomplete inspections, include counts in JSON, and make retained conflicts produce the issue exit status with a review command.
  - [x] Distinguish actionable conflicts from superseded immutable history. Successful syncs and replacement conflicts durably supersede obsolete attempts, exclude them from active daemon/direct status counts, reject attempts to resolve historical inputs with actionable guidance, and print exact editor/local/remote resolution commands from conflict list and detail output.
  - [x] Make `daemon status` project useful per-vault operational state: reconstructed daemon sync state, latest daemon-supervised attempt time, and cached notification-advertisement discovery with secret-safe endpoint identity. Inspection must be read-only, work while the daemon is stopped, avoid network refresh, and distinguish never attempted, not discovered, disabled, and unavailable.
  - [x] Distinguish a stopped daemon (no runtime metadata) from an existing runtime that fails its authenticated capability probe. Preserve the probe failure in JSON and render the latter as unresponsive rather than falsely claiming the daemon stopped during startup, load, or service failure.
  - [x] Preserve the sync engine's stable error category and retryability through `vulcan-app` into retained daemon jobs instead of flattening engine failures into retryable `unknown` errors; keep explicit cancellation authoritative.
  - [x] Make quiet installed daemons log terminal job failures once per wiki/failure state without leaking error detail, retain full detail on authenticated status, and clear deduplication after recovery. Keep branch-lane failures on their existing specific diagnostic.
  - [x] Promote daemon attention outcomes into structured warning/error events for conflicted, paused, and failed sync jobs, with per-wiki state deduplication and recovery reset. Keep operational logs always on and add opt-in, timeout-bounded, shell-free native desktop delivery through `daemon config set-notifications --desktop true`, including `termux-notification` on Android; retain desktop delivery identity in the durable alert ledger so enabling it after a failure surfaces the current state once without restart spam. Scheduled finite sync runs remain the normal Termux deployment. Delivery failures must never affect sync state or block the sync worker. Native daemon notices now expose secret-minimal job identity and retryability, and device-local policy can ignore network failures or alert once after a configured failed-job count or observed duration; logs and remote sinks remain independent.
  - [x] Add durable outbound delivery for remote notification sinks. Versioned secret-minimal events use stable job identities, a bounded persistent ledger, startup reconciliation, bounded retry/backoff, and explicit `daemon alert-status` health. First-party JSON webhook and ntfy delivery use HTTPS (or loopback HTTP), no redirects, optional environment-only bearer tokens, and per-wiki network permission checks. Shell-free absolute command adapters receive JSON on stdin for local email/NATS bridges and reapply execute permission. Delivery success never becomes part of sync correctness.
- [x] On Android, support one-shot execution under Termux before requiring an always-running daemon or custom app. `vault clone --git-dir <termux-private-path> --platform android-shared` creates and registers the detached layout, and ordinary `sync status`, `sync doctor`, `sync run`, conflict review, and recovery commands use the same direct application workflows without starting the daemon. Integration tests exercise a modified shared-storage worktree through a one-shot push and prove no daemon runtime is created.
  - [x] Add an energy-aware Termux packaging adapter over that same finite command. `sync termux-install/uninstall` dry-runs explicitly, requires a registered detached `android_shared` Git wiki, atomically manages a private executable wrapper and bounded manifest, derives a stable nonzero job ID unless overridden, and calls Android's JobScheduler with network, battery-not-low, storage-not-low, optional charging, minimum-period, and reboot-persistence constraints. Its finite wrapper posts a stable high-priority `termux-notification` for non-network errors and applies a private, durable immediate/ignore/count/duration policy to network failures, then removes the notice and resets failure state after recovery without changing the sync exit status. It never starts the daemon or adds a second sync engine; foreground shortcuts and a future native bridge may invoke `sync run` more promptly while the periodic job remains a low-frequency safety net.

- [x] Add `sync schedule show/set` to inspect saved Termux job settings and change the interval, network, charging, battery, reboot, or network notification policy while preserving unspecified settings and the existing job ID. Support JSON and mutation-free previews, enforce Android's minimum interval, and distinguish saved settings from the live Android job queue.
...
- [x] Add deterministic sync performance gates that measure top-level Git process work instead of flaky wall-clock timing. An unchanged cycle uses the persistent private index and hidden local ref to bypass full capture and its redundant `HEAD` lookup, is limited to one remote query, no redundant fetch or accepted-ref rewrite, cached-stat verification, cached filter-path and immutable-tree platform analysis, batched repository discovery, requirement-environment inspection, and hidden-ref reads, and at most 23 Git subprocesses even with fresh Git and Git LFS readiness probes. Long-running daemon workers reuse the validated Git installation across timeout-specific engine clones and reduce that ceiling to 22; changed worktrees retain the stable two-pass capture, platform caches are keyed by immutable revision and exact policy, and changed accepted refs retain one atomic transaction. Stuck Git/filter/transport process groups have a tested configurable timeout, and direct interactive progress is transient by default with durable diagnostics behind `--verbose`.
  - [x] Precompute case-collision and structural-diagnostic membership once per conflict with a literal multi-pattern scan. Classification no longer lowercases and scans the complete conflict path/diagnostic set for every path; a 10,001-path reordered fixture asserts one normalization per path and one pass over diagnostic lines.
  - [x] Stream batched Git blob responses header-first with explicit per-object and aggregate limits. Caller-specific bounded path reads account for repeated paths before cloning shared objects, and agent proposal input threads its 32 MiB budget across base, local, and remote reads before provider invocation.
  - [x] Bound the initial serialized conflict response independently of in-memory evidence. Backend, durable-record, and projected worktree-application summaries return counts, completeness flags, at most 16 path/classification/application previews, 4,096 diagnostic characters, and `detail_conflict_id`; large-path serialization tests keep JSON below fixed ceilings while paged `sync conflicts <id>` remains the complete inspection surface.
  - [x] Expose versioned content-free per-cycle operational stats for automatic/conflicted/grouped/formatting-candidate paths, logical preserved-input bytes, transaction-local Git subprocesses, and coarse application stages. Daemon jobs receive independent collectors; stats add no Git calls or evidence reads and never affect synchronization identity or durable state.
  - [x] Exercise a real installed-Git conflict storm with 1,000 paths in ordinary tests and 10,000 paths in the scheduled hardening target. The shared lifecycle mixes formatting and semantic conflicts with a structured automatic merge and clean edits, verifies bounded initial/page responses and subprocess work, resolves 128-group batches across state-store reopening and unrelated live advancement, preserves immutable evidence and the normal index, and either drains exactly or leaves the larger session durably actionable.
...
- [x] Add platform smoke tests that download the just-built archive into a clean environment, verify it, run `vulcan --version` and representative direct-vault commands, confirm `sync doctor` reports the Git dependency truthfully, and exercise service-plan dry runs. Keep Android/Termux as its existing separately tested package/runtime profile rather than relabeling a desktop Linux archive as Android support.
  - [x] Publish a distinct full-feature `aarch64-linux-android` archive built with the Android NDK, including QuickJS bindings generated against the NDK sysroot plus OAuth, vectors, and web support. Apply the verified AArch64 LLVM SLP-vectorizer workaround, verify the cross-built AArch64 ELF and linked QuickJS entry points, include the archive in manifests, checksums, signed update metadata, and rolling releases, and make the POSIX installer select it only after detecting Termux. Ship a Termux-only runtime smoke script for QuickJS, vectors, and sync diagnostics without treating the cross-build or script availability as completion of the separate real-device certification gate.
...
- [x] Add one clearly labelled rolling nightly or development prerelease only when `main` advanced since the last published development build. Prefer a single scheduled eligibility check plus manual dispatch, reuse the canonical package builders, and replace or prune the previous rolling release instead of accumulating permanent nightly tags and assets. Publish it under the distinct `rolling-main` channel tag, created once and never moved so fetches never collide with the `main` branch, and replace only that release's assets in place.
...
- [x] Add vault-independent unattended portable updates through `vulcan self-update schedule install/show/uninstall` and the non-interactive `self-update run` cycle. Project a daily local-time job into `systemd --user`, a macOS LaunchAgent, or a current-user Windows scheduled task, and use a persisted battery/storage-aware periodic Termux JobScheduler job on Android where exact wall-clock execution is unavailable. Keep trusted signatures mandatory, support stable or explicitly selected `main` channels and HTTPS descriptor overrides, provide mutation-free plans and bounded device-local manifests, and optionally notify through native platform helpers after failures. Download and verify the complete archive before touching a live daemon; reject unresponsive runtime state, perform graceful final-sync shutdown immediately before replacement, and restore either the installed native daemon service or the prior detached runtime after success or replacement failure.
- [x] Add a reusable policy for background network and power constraints. Apply it to unattended self-update before network work and before archive download, project supported constraints into Termux JobScheduler and Windows Task Scheduler, and report deferred cycles with observed conditions. Keep explicit unknown-state behavior and preserve old schedule manifests.
- [ ] Extend the background-work policy to automatic sync after defining separate defaults for finite manual runs, daemon-triggered cycles, and Termux jobs.
...
- [x] Implement canonical legacy/key-shaped ID parsing and public-key-derived ID generation for the recovery reader and local identity store. Live sync writers still use the legacy actor ID until the version-3 gate is complete.
...
- [x] Preserve a captured local safety backup and reject unknown future `Vulcan-Ref-Namespace` values on the remote live tip before canonical reconciliation; document that current version-2 writers still use legacy ULIDs and reserve version 3 for key-derived IDs.
- [x] Add a strict key-ID recovery reader for device backup inventory, labels, fetch, and pruning while keeping legacy-ID creation, version-2 writer provenance, and the version-3 live-tip reconciliation block. Surface `ssh_key_v1` versus `legacy_ulid` as identity form, never trust.
- [ ] Enable version-3 live-tip reconciliation only after every snapshot, merge, bridge, and epoch descendant preserves the maximum observed namespace version; fresh legacy-ID writers may continue version 2. Test that a version-3 tip can never be followed by a version-2 descendant that would mislead old readers.
- [ ] After compatible reader deployment, switch new device identity creation and its ref/trailer provenance to key-derived IDs and namespace version 3 while retaining mixed legacy/key recovery heads and old ref paths.
...
- [ ] Test a real mixed-version migration with multiple bare-remote clones: legacy readers/writers, the dual-reader release, key writer activation, conflict preservation, old-head recovery, exact-lease cleanup, offline resumption, and refusal by binaries that cannot interpret namespace version 3.
...
#### 12.17.6 Execution authorization and embedded isolated sigchains

**Consumer contract:** [Scheduled execution](specs/scheduled-execution.md). Extend the accepted-registry verifier and 10.8 authorizer, not a second key-management system. Static-key scheduling ships independently.

- [ ] Define a typed, scoped execution-authorizer role and exact commit/configuration payload contract, including its relationship to dedicated registry-administration keys and device keys. Enrollment or automatic sync signatures alone never authorize execution. Reuse the static-key verifier result/evidence interface and preserve sandbox/resource grants.
- [ ] Add an accepted-registry adapter bound to exact registry ID, checkpoint, vault, and execution scope. Recheck known revocations at dispatch and mutation boundaries; atomically invalidate affected authorization and request cancellation. Use last accepted authority offline without mandatory leases; report unbounded remote-revocation delay and deny affected work if an accepted revocation cannot be activated.
- [ ] Specify a versioned embedded transport profile allowing multiple isolated public sigchains in any ordinary Git repository or vault. Keep current dedicated-repository registry v1 supported. Define paths/ref or immutable-record representation, complete authenticated linear transition evidence, and transport through ordinary Git and Vulcan file-tree sync without imposing registry single-parent history on the outer wiki repository.
- [ ] Pin each chain's immutable ID/root/checkpoint and consumer scope independently outside synced authority. Discovery grants no trust; never union chains, accept unqualified principal collisions, or bootstrap from a candidate's own keys. Preserve parent-admin eligibility, rotation/removal, pending-versus-accepted history, fork/rollback rejection, and explicit recovery.
- [ ] Test multiple chains in one vault/repository, cross-chain substitution, malicious bootstrap, missing/invalid intermediate history, concurrent transitions, ordinary outer merges, delayed revocation, rollback/restored checkpoints, and failed projection. Document static-to-registry migration with no broadened authority and unchanged schedule definitions; update skills only when commands ship.
...
### 12.19 Deterministic artifact mergers and Git integration

**Goal:** Reuse one validated deterministic artifact-merger registry for Vulcan's internal file-tree synchronization and for ordinary Git repositories. Automatically merge only formats whose semantics make the exact conflict convergent; keep malformed, incompatible, structural, or identity-ambiguous inputs recoverable and reviewable.

**Depends on:** The implemented 12.5 structured merge and conflict-preservation boundary. Individual format adapters depend on their owning format contract: Phase 19.13.4 for event sets, 19.13.5 for replicated relational changes, and 19.13.6 for canonical Automerge documents. Phase 14/16's note-editor Automerge state remains a separate device-local crash-recovery concern unless deliberately promoted through 19.13.6.

- [ ] Promote the structured merge dispatch into a closed, versioned `ArtifactMerger` registry shared by internal sync and plumbing callers. Each built-in registration declares exact path/media/format versions, commutativity and side-role independence, base requirements, deletion/compaction semantics, maximum bytes/items/depth, canonical output rules, and post-merge validation/evidence; packages may select a registered format but may not supply native/WASM/JS merge code during sync.
- [ ] Prefer conflict-avoiding layouts for append-only data: store immutable database changes, durable events, and similar operations under actor/sequence- or content-ID-named paths so ordinary Git performs directory union. For packed files, admit only validated stable-ID set union with canonical ordering and duplicate elimination. Reject the same identity with different payload bytes, gaps that violate the declared checkpoint/sequence contract, incompatible schema, bad digest/signature, unsafe compaction lineage, or physical delete/modify.
- [ ] Add adapters only alongside pinned format specifications and hostile conformance fixtures. Initial candidates are the Phase 19 replicated-store changeset/event envelope and, if it ever becomes canonical file-tree data, pinned Automerge document states. A CRDT adapter must prove byte-stable or canonically normalized, commutative, associative, and idempotent merge across role/order permutations, repeated merges, logical deletion, stale replicas, compaction/checkpoints, malformed input, and resource limits before its policy can default to automatic.
- [ ] Route Phase 12's immutable base/local/remote objects through the registry before conflict materialization, retaining exact merger/version, input and output digests, validation checks, and policy identity in the structured sync report and Git-reachable provenance. Never let a format merger update refs, the normal index, worktree files, cache state, or external services directly.
- [ ] Add a non-interactive Git low-level-driver plumbing command over that registry with explicit base/current/other/path and format inputs. It leaves Git's current temporary file byte-identical unless a result fully validates, atomically replaces it on success, emits diagnostics only on stderr, and follows Git's contract: exit `0` for resolved, nonzero for an unresolved content conflict, and `>128` for driver failure. Since Git does not provide immutable candidate commit IDs to external drivers, expose only mergers proven independent of local/remote labels and arrival order; keep identity-tiebreaking JSON/Markdown mergers internal.
...
- [ ] Optional session persistence: store Automerge binary doc in device-local `.vulcan/` state for crash recovery, exclude it from file-tree sync, and discard it after successful materialization. If a later feature promotes Automerge state to a canonical synchronized artifact, pin its on-disk/version contract and qualify its deterministic merger through 12.19 rather than treating crash-recovery bytes as canonical.
...
- [x] Persist locked, versioned Outline route run checkpoints with stable operation IDs and running/completed/failed outcomes under `.vulcan/integrations/routes/` on desktop or private per-vault state on Android, outside the rebuildable cache.
- [x] Initial Outline pull slice: persist locked, versioned remote-ID/local-path mappings and local/remote/base content hashes under `.vulcan/integrations/outline-pull/` on desktop or private per-vault state on Android, with cross-platform atomic state/snapshot replacement, Unix directory durability syncing, and fail-closed validation.
...
- [ ] Store per-route state under a locked, ignored `.vulcan/integrations/` state area on desktop or private per-vault state on Android, outside `cache.db`, using validated schemas, versioning, verified temporary files, `fsync`, and atomic replacement. A malformed or unsupported state file stops the route without local or remote mutation.
...
- Beyond stable v1, model immutable event sets, replicated relational change sets, and Automerge documents as distinct canonical store formats rather than generic binary artifacts. Each may have a faster device-local materialization at an exact canonical frontier; the materialization is rebuildable acceleration, while synchronized events/changes/documents and any authoritative compaction checkpoint remain the source of truth. Format-specific APIs and conformance gates must preserve the distinction instead of forcing non-document application data into Markdown/mdbase.
...
- [ ] Apply 10.8 and `specs/scheduled-execution.md` to schedule ownership and execution trust. Approval means effective capability authority plus the configured repository/commit policy, not a second device-local enable switch or recurring confirmation. Canonical vault definitions select nodes; immutable verified configuration and package snapshots survive reload without substituting running code. Retain daemon availability and existing runtime ceilings.
...
### 19.13 Application storage, canonical artifacts, event stores, and replicated datasets

Store authority and physical acceleration are separate. A canonical event/change/document representation may be synchronized while each device maintains a derived KV, SQLite, cr-sqlite, or Automerge materialization at an exact accepted frontier. Materializations must be incrementally advanceable and fully repairable from retained canonical inputs; an unreconstructable local database is authoritative device state, not a cache. Stable App v1 does not accept these event, replicated-relational, or Automerge-document profiles; the post-v1 gates below must freeze each format and its semantics first.
...
- [ ] Use one mdbase persisted typed-collection model and one Vulcan SQLite execution/mutation engine across App and script bindings; expose native conveniences only with explicit collection semantics and the same query optimization/write validation as canonical mdbase frontends
...
#### 19.13.3 Canonical allocation and common frontier/materialization contract

- [ ] Define a host-owned canonical data allocation for non-note App stores inside the synchronized vault, bound by instance/store identity and included in history, backup, storage accounting, permissions, and Phase 12 sync while excluded from ordinary note scanning, Obsidian presentation, and publication. Keep its exact root and filenames host-assigned; never reuse the private `.vulcan/apps/<instance>/cache|state` allocation.
- [ ] Separate `canonical_format`, `materialization_engine`, and `projection` from authority/scope/replication in the experimental store contract. A store report exposes accepted canonical frontier, local materialization frontier, lag, schema/adapter/reducer versions, integrity, rebuild/repair state, retained operation/checkpoint bytes, and compaction eligibility without leaking private paths or unauthorized records.
- [ ] Key every derived materialization by store identity, exact canonical frontier, schema version, adapter version, reducer/projection content identity, and relevant configuration. Incrementally apply accepted operations transactionally; on missing inputs, digest/frontier mismatch, incompatible code/schema, or interrupted advancement, retain diagnostics and rebuild from an accepted checkpoint plus operations rather than guessing.
- [ ] Provide bounded snapshot/replay APIs so an App can query a coherent materialization while a newer frontier is being built. Publish a new materialization atomically only after schema and adapter validation; retain or recreate the previous accepted view across crashes.
- [ ] Treat checkpoints as acceleration while all required canonical history remains. Promotion followed by event/change deletion is explicit journaled compaction with a plan, frontier proof, offline-replica and retention horizon, backup/recovery evidence, permission checks, and interruption-safe rollback/roll-forward behavior.

#### 19.13.4 Immutable event sets and materialized projections

- [ ] Specify a versioned Vulcan event-set format for authoritative application/domain events: store/schema identity, stable event ID, actor/sequence and optional causal prerequisites, canonical payload encoding, timestamp semantics that never decide merge winners, payload digest, optional signature, and typed attachment/blob references. Distinguish these durable domain events from daemon delivery notifications and lifecycle hooks.
- [ ] Prefer one immutable event record per actor/sequence- or content-ID path so Git merges concurrent creation without a content driver. Define bounded canonical packs and pack indexes for small-file efficiency; a pack is equivalent to its event set, duplicate IDs with identical bytes deduplicate, and the same ID with different bytes is corruption/conflict.
- [ ] Define reducer/projection contracts with input and output schemas, deterministic code identity, checkpoint compatibility, maximum work/state, and explicit handling for concurrent incomparable events. Set union preserves facts but does not make every reducer semantically conflict-free; noncommutative or prerequisite-violating histories surface typed application conflicts after transport merge.
- [ ] Add reusable host-managed incremental projection caches for Apps, including exact processed frontier, per-pack/event verification, transactional advancement, secondary indexes, invalidation on reducer/schema/config change, bounded replay, rebuild progress/cancellation, and cache-integrity repair. Reducer execution happens after accepted sync materialization and never inside Git merge, checkout, or conflict inspection.
- [ ] Add direct and daemon App API/CLI administration for event append, bounded read/tail/query through declared projections, frontier/lag/status, rebuild/verify, checkpoint, compaction plan/apply, export, and recovery. Writes use stable idempotency keys and never acknowledge an event before its canonical record is durable.

#### 19.13.5 Replicated relational change sets
...
- [ ] Specify a versioned replicated-change envelope, sharing immutable-record storage/packing primitives with event sets while retaining distinct adapter semantics: replica/sequence range, source and resulting frontier/checkpoint, schema and adapter format versions, payload digest/signature, dependencies, duplicate rules, and explicit collision/gap/compaction failures.
...
- [ ] For a cr-sqlite profile, keep the writable replica database as a host-owned local materialization and emit/import bounded immutable change batches plus accepted checkpoints; do not Git-sync a live SQLite/WAL file or let the local replica become the only authoritative copy. Apply newly accepted batches transactionally, expose frontier/lag/conflicts, and rebuild from checkpoint plus retained batches.
...
- [ ] Implement any accepted replicated-store artifact merger through the shared Phase 12.19 registry over immutable base/local/remote inputs, declared schema/version, strict resource limits, exact output validation, and evidence reports; never execute arbitrary app merge code merely because synchronization discovered a conflict

#### 19.13.6 Automerge application documents

- [ ] Define a separate experimental `automerge-document` store for bounded document-shaped App state (maps, lists, text, and scalars) that needs offline concurrent editing but not relational queries or an application event reducer. Keep it distinct from Phase 14/16 note-editor sessions and from daemon live-session presence state.
- [ ] Pin the Automerge library and on-disk/state exchange version, actor/device identity mapping, schema envelope, document/change/byte limits, attachment references, logical deletion, compaction/history retention, import/export, and upgrade behavior. Split unrelated records across documents rather than growing one vault-wide CRDT.
- [ ] Decide and test the canonical representation: a complete mergeable document state, immutable Automerge changes plus a head manifest, or both with one declared authority. Require deterministic content identity or canonical serialization, commutative/associative/idempotent state merge, role/order permutation parity, and exact convergence after replay before enabling the 12.19 Git driver.
- [ ] Maintain optional host-owned query projections keyed by Automerge heads plus schema/projection identity. Projection failure or absence must not block preservation/sync of a valid document and must be repairable without changing canonical CRDT history.
- [ ] Expose bounded document transaction, heads/history, merge evidence, snapshot/compact, verify/rebuild, export, and conflict-recovery operations through shared direct/daemon App services. Invalid state, unsupported history, physical delete/modify, and resource exhaustion preserve every input rather than selecting a side.

#### 19.13.7 Storage selection, verification, and reference coverage

- [ ] Publish a storage-selection matrix and authoring guidance: use mdbase/Markdown for user-readable linked knowledge; atomic artifacts for serialized/single-writer specialized files; event sets for append-only history, audit, workflows, and reducer-shaped domains; replicated relational changes for multi-writer transactional/query-heavy data; Automerge for smaller document-shaped concurrent state; blobs for immutable large values; live sessions for expiring coordination; and private stores for non-shared device state/caches.
- [ ] Add one bounded reference fixture/app slice for each special canonical shape rather than forcing every first-party app onto mdbase: an auditable event/projection example, a relational multi-writer example when an adapter passes conformance, and a small Automerge document example. Keep the Personal Ledger's stable v1 canonical SQLite baseline until the relational profile passes its post-v1 gate.

- [ ] Add crash/restart, concurrent mutation, cache reset, incremental materialization/rebuild, reducer/adapter upgrade, frontier gaps, event-ID collisions, stale checkpoints, compaction horizon, local-state retention/deletion, migration, quota, backup/restore, WAL cleanup, corrupt database, canonical sync conflict, Automerge convergence, live-session expiry, blob GC, and package-uninstall-with-data-preserved integration tests
...
- [ ] Prove the primary 10.8 use case: canonical subscriptions/capture rules, a vault-assigned refresh node, RSS articles captured as Markdown, and portable authoritative capture bindings outside `cache.db`. Fetch/prepare outside mutation locks, recheck note/binding revisions before journaled apply, and advance checkpoints only after recoverable content/binding persistence. Default to import-once; source updates require explicit user-edit-preserving policy.
...
- [ ] Use canonical mdbase-compatible subscription definitions/capture rules for vault-owned schedules; retain device-local subscriptions only for explicitly local/manual use. Keep authoritative capture bindings portable under 10.8, unread/star/archive UI state as durable local typed state, fetched bodies/indexes as bounded SQLite-derived cache, enclosures as policy-bound content-addressed blobs, and saved entries as ordinary canonical Markdown with user-facing source/provenance fields.
...
- [ ] Test reassignment after syncing notes/bindings, offline old/new runner overlap, interrupted content/binding writes, and signed capture-rule changes. Preserve conflicts and user edits without claiming cross-machine exclusion. No private-only deduplication authority or independent feed timer.
...
- [ ] **Post-v1 event-store gate:** Keep event sets experimental until 19.13.3–4 freeze the canonical envelope, packing, frontier, reducer/projection, checkpoint, and compaction contracts and an auditable fixture proves deterministic incremental materialization, full rebuild, concurrent append, malformed/colliding input handling, and sync/merge-driver behavior
- [ ] **Post-v1 replicated-relational gate:** Keep replicated relational stores experimental until 19.13.3 and 19.13.5 demonstrate one explicitly versioned adapter profile across offline multi-writer behavior, relational invariants, schema migration, malicious inputs, revocation, recovery, compaction, evidence, and transport/runtime compatibility; do not block the core app platform on this investigation
- [ ] **Post-v1 Automerge-document gate:** Keep canonical Automerge App stores experimental until 19.13.3 and 19.13.6 freeze one bounded representation and prove state/change convergence, deterministic identity/serialization, schema and actor policy, incremental projections, compaction, hostile inputs, and Phase 12.19 internal/Git merger parity. This does not gate ephemeral Phase 14 editor sessions.
...
- [x] Preserve explicit relative link meaning in shortest-mode indexed and
  direct resolution; verify generated wiki references when same-named notes
  coexist and leave missing explicit paths unresolved instead of retargeting.
...
- [x] Repair full-book import routing: preserve authored anchors under alternative outlines, reject unsupported section tails, retain validation warnings and root titles, flag large root remainders, and resolve plain references conservatively across all overlapping note spans. Clarify MDAF v1 and bundled agent guidance; validate new producer derivatives end-to-end.
...
- [x] Apply `collection.read_defaults` only to missing effective fields while preserving missing, explicit null, empty string, and empty list as distinct states; validate JSON Schema `required` against persisted frontmatter only.
...
- [x] Review the CEL engine's transitive `paste` maintenance advisory and scope the supply-chain exception to `RUSTSEC-2024-0436` for compile-time identifier generation.
- [ ] Remove the `RUSTSEC-2024-0436` exception when the CEL dependency can drop the unmaintained `paste` crate.
...
- [x] Route App/script and generic managed note/property/task edits affecting collection records through that same mdbase-validating pipeline, accounting for old/proposed membership; define explicit raw repair with normal permissions/recovery and resulting diagnostics, never silent fallback on validation failure. Add cross-entrypoint write parity tests before claiming unified routing.
...
**Contract:** [MDB performance and native integration](specs/mdb/PERFORMANCE_AND_NATIVE_INTEGRATION.md). Use one mdbase persisted typed-collection model, one Vulcan SQLite execution/mutation engine, and multiple frontends for Apps and standalone scripts. Keep mdbase first-class regardless of adoption forecasts. Native query conveniences preserve explicitly bound collection semantics; no second collection format, schema language, or independently maintained engine is introduced. Existing ordinary native note queries retain their semantics.
...
- [x] Clarify mandatory SQLite-backed execution, equal optimization across collection-bound query frontends, and one mdbase-validating Vulcan write path; native binding and managed/raw-edit routing remain unchecked implementation work.
...
- [ ] Implement the initial SQL lowering subset for type membership, path prefixes, scalar comparisons, conjunctions, selected fields, and ordering through one physical planner; require indexed execution plus semantic evidence, retaining residual CEL and exact totals/grouping without premature pagination. Canonical mdbase queries must receive the same optimization as collection-bound native equivalents.
...
- [ ] Exercise one shared inspect/schema/read/query service through a shell task-list script, Python structured integration, and Collection Studio bindings; retain canonical mdbase envelopes and existing unbound native-query semantics, with no App package or daemon required for standalone scripts. Review native-query binding/input/output/capability contracts explicitly before exposing them, and test cross-frontend result, diagnostic, physical-plan, and latency parity.
...
| `automerge` (exact reviewed pin for canonical storage) | CRDT document model for collaborative editing and the separately gated bounded App document store | 14, 16, 19.13.6 |
...
| SQLite Session support through the existing bundled SQLite boundary | Candidate immutable relational changeset capture/apply adapter | 19.13.5 (experimental) |
| `cr-sqlite` (optional, exact-pinned and statically linked) | Candidate multi-writer relational materialization/change adapter; never package-supplied | 19.13.5 (experimental) |
