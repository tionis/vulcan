...
- [x] Add deterministic sync performance gates that measure top-level Git process work instead of flaky wall-clock timing. An unchanged cycle uses the persistent private index to bypass full capture, is limited to one remote query, no redundant fetch, one atomic accepted-ref transaction, cached-stat verification, cached filter-path analysis with exact attribute-source invalidation, batched repository discovery and requirement-environment inspection, and at most 27 Git subprocesses even with a fresh Git LFS readiness probe; changed worktrees retain the stable two-pass capture. Stuck Git/filter/transport process groups have a tested configurable timeout, and direct interactive progress is transient by default with durable diagnostics behind `--verbose`.
