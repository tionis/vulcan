...
- [ ] `vulcan --help` (clap) should match the grouped layout — currently lists 37 commands in alphabetical order with no visual hierarchy
...
- [ ] Prioritize commands users encounter first: `note get`, `query`, `search`, `daily`, `tasks list`, `run`
...
- [ ] **`note info <note>`** — summary view: path, word count, heading count, link/backlink counts, tag list, frontmatter keys, created/modified dates. Quick overview without reading content. `--output json` for scripting.
- [ ] **`note history <note> [--limit <n>]`** — git log scoped to a single note file. Shortcut for `git log -- <path>` with formatted output showing commit message, date, and diff stats.
...
- [ ] **`vulcan tags [--count] [--sort count|name] [--where <filter>]`** — list all tags in the vault with occurrence counts. Filterable to a path prefix or property condition. `--output json` returns `[{ tag, count }]`.
...
- [ ] Add `--quiet` / `-q` global flag — suppress scan progress, warnings, and non-essential stderr output. Only errors and primary output remain.
- [ ] Respect `VULCAN_QUIET=1` environment variable as equivalent to `--quiet`

**Table output control**

- [ ] Add `--no-header` flag for table/TSV output — suppress column headers for piping to `cut`/`awk`
- [ ] Add `--format tsv` output mode — tab-separated values, easy for shell pipelines. Complement to `--output json` which is heavy for simple field extraction.
- [ ] `--format csv` as a direct output mode (not just `--export csv` which writes to a file)
...
- [ ] Standardize exit codes across all commands: 0 = success, 1 = error, 2 = issues found (doctor, automation)
- [ ] Add `--exit-code` flag on query/search commands: return exit code 1 if zero results. Useful in conditionals: `if vulcan query --where 'status = blocked' --exit-code; then ...`
