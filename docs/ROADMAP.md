...
- [ ] Extract remaining MCP command dispatch dependencies on CLI handlers/options into shared app workflows and transport-neutral protocol types. Choose a shared protocol module/crate in the 10.7.1 record if needed; neither daemon nor app may import CLI. Preserve tool names, schemas, report/error shapes, resources, prompts, pagination, and permission filtering.
  - Request argument types and protocol constants live in `vulcan-app::mcp_protocol`; tool JSON Schemas live in `vulcan-app::mcp_schemas`; the built-in tool catalog, pack selection, annotations, and permission visibility live in `vulcan-app::mcp_catalog`. `vulcan-app::mcp_dispatch` owns JSON-RPC request framing behind a method-handler boundary; `vulcan-app::mcp_assistant` owns permission-filtered prompt/skill discovery, prompt rendering, and vault-owned assistant resource reads; `vulcan-app::mcp_completion` owns permission-filtered completion and response shaping with the host's help-topic catalog injected. `vulcan-app::mcp_read_tools` owns the search and query workflows plus the note-source read boundary; `vulcan-app::notes` now resolves vault and direct Markdown targets for both CLI and future hosts. Both current transports use these shared contracts. Remaining note-read report workflows and tool methods, resource listings/help/custom-tool resources, the shared help-topic catalog, and HTTP/OAuth routing still need extraction.
