...
- [ ] Introduce ref namespace version 3 for the expanded device-ID grammar while retaining the same device-head path; current writers already emit version 2 with legacy IDs. Ship a first compatibility release that reads namespace versions 1/2/3 and legacy/key device IDs but still creates legacy IDs; switch writers to key identities only in a later release after confirming reader deployment. Current version-2 readers reject unknown future live-tip versions before canonical reconciliation.
