...
- [x] Add platform smoke tests that download the just-built archive into a clean environment, verify it, run `vulcan --version` and representative direct-vault commands, confirm `sync doctor` reports the Git dependency truthfully, and exercise service-plan dry runs. Keep Android/Termux as its existing separately tested package/runtime profile rather than relabeling a desktop Linux archive as Android support.
  - [x] Publish a distinct full-feature `aarch64-linux-android` archive built with the Android NDK, including QuickJS bindings generated against the NDK sysroot plus OAuth, vectors, and web support. Apply the verified AArch64 LLVM SLP-vectorizer workaround, include the archive in manifests, checksums, signed update metadata, and rolling releases, and make the POSIX installer select it only after detecting Termux. This is packaging support, not completion of the separate real-device certification gate.
