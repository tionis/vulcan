...
- The first mutating sync atomically creates one Crockford-Base32 ULID in device-local sync state and reuses it as the Git actor identity across vaults; dry-run and doctor only read it and never initialize state. Vulcan-created live snapshot, live merge, recovery snapshot, and conflict-resolution commits use concise non-semantic subjects plus parseable trailers for sync protocol version, device, profile where applicable, policy version/hash, immutable source revisions, conflict identity where applicable, and `Vulcan-Sync-Semantic: false`. This provenance is Git-reachable and never depends on `cache.db` or note frontmatter.
