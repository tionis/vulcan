...
- **Realtime Git notification model:** Consume a generic CloudEvents-based event-relay protocol and forge-neutral Git ref-update profile. The relay is not Vulcan-specific, Git remains authoritative, notifications only enqueue the ordinary sync transaction, and polling remains the repair path. The draft contracts live in `docs/specs/event-relay-protocol.md` and `docs/specs/git-realtime-events.md`.
...
- Remote Git wakeups use the generic Event Relay Protocol and Git Realtime Events profile in `docs/specs/`, not the loopback companion protocol and not a Vulcan-specific public relay. A dependency-light model boundary validates source descriptors, secret-bearing subscription bundles, CloudEvents, and Git event profiles without network I/O. The daemon owns asynchronous transport connections, reconnect/backoff, retained-event acknowledgment, and source/ref routing; a validated matching event can only enqueue `SyncJobTrigger::RemoteNotification`. It cannot call Git, select a local path from event data, resolve a conflict, or replace periodic reconciliation. Subscription descriptors, explicit repository/ref bindings, cursors, and diagnostics are device-local durable state outside `cache.db`; capability tokens remain in a platform secret store or permission-restricted secret file. The reference relay is planned as an independent generic CloudEvents project, as detailed in `docs/specs/event-relay-implementation-plan.md`.
