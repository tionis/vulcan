...
- Agent-assisted resolution begins at a provider-neutral `vulcan-app` boundary only after a durable conflict exists. The synchronous provider receives bounded exact base/local/remote bytes plus policy identity and explicitly permitted focused context; immutable sides are inventoried in three batched Git reads independent of selected path count, and the aggregate budget is enforced during assembly; named permission profiles gate Git and every exposed path, internal Obsidian/Vulcan state and binary/missing inputs are excluded, and broad context is an explicit capability rather than ambient filesystem access. Explicit focused files are symlink-safe UTF-8 reads, sorted and deduplicated, sent with exact bytes and hashes, and retained as hash/size proposal inputs. A provider-neutral tool boundary adds fixed-call/fixed-byte `vault_read`, `vault_search`, `vault_query`, and `vault_links` operations; indexed operations apply `PermissionFilter` before materialization and policy hooks before disclosure, while full-file reads outside explicit context additionally require the caller's `--allow-broad-context` opt-in. The OpenAI-compatible adapter handles bounded tool turns; the distinct reviewed supplied-file path is local and performs no network or tool access. Proposal identity binds tool names, exact argument/result hashes, and returned paths without retaining tool-result content. Proposal format version 4 also reserves an optional immutable complete-group selection and accepted-frontier binding in that identity, while retained version-3 records continue to receive their original integrity verification. A provider cannot claim a context path that was neither supplied nor returned by a tool. Output must resolve exactly the conflicted paths within byte limits and cannot request deletion. Vulcan constructs an unreferenced proposal tree, rereads every blob and mode, renders a bounded patch, verifies the worktree and all preservation refs remained unchanged, and runs the same effective-config whole-tree link/deletion validator used by deterministic merging before atomically retaining a versioned `ResolutionProposal` outside the vault/cache. The proposal records explanation, referenced context, provider/model, prompt/tool contract versions, immutable conflict/policy inputs, path hashes, and validation evidence. Generation holds the repository mutation lock, is cooperatively cancellable, permits one retained proposal per conflict, and never writes the worktree or any ref. Approval has a read-only preview and a separately locked application transaction: it rejects stale inputs, unsafe Git state, moved live refs, invalid syntax, altered objects/patches, changed whole-tree validation policy, newly broken links, excessive deletions, an explicit rejection record, or a tree that cannot be reconstructed from the preserved merge; captures recovery state before remote access; publishes only with a lease; applies only the accepted proposal commit; and records a deterministic audit event without prompts, explanations, patches, or note content. Rejection is a separate previewable, idempotent, repository-locked transaction which retains the proposal and all original objects, writes only a deterministic content-free audit decision, and permanently closes that proposal ID against approval. The direct CLI exposes generation through `sync propose`, rejection through `sync reject`, exact approval through `sync resolve --approve-proposal`, and the composed manual transaction through `sync resolve --file`. `sync propose --auto-accept` is a deliberate two-factor policy exception to human review: it is rejected before provider execution unless the caller also enabled device-local `sync.agent_auto_accept`, then persists the proposal and runs the exact ordinary approval transaction; failures leave the proposal reviewable and success uses a distinct content-free `auto_accepted` audit action. Credentials come only from an explicitly named environment variable, while endpoint access is permission-gated and responses are bounded exact JSON. Configurable retention, companion claiming, and additional provider protocols build on these boundaries rather than bypassing them.
