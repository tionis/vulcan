...
- Unresolved Git merges receive deterministic conflict identities derived from the versioned policy hash, merge base, order-independent candidate tips, and sorted conflicted paths. Structured concurrent additions likewise order by immutable candidate object identity, so swapping local/remote roles or trigger arrival order cannot change a resolvable tree. No wall-clock timestamp, arrival position, or model confidence participates in winner selection. Before a conflict is surfaced, base/local/remote commits are anchored under create-only `refs/vulcan/conflicts/<id>/` refs. For bounded regular-file conflicts with a unique base, Vulcan also constructs a safe materialization: normal clean merge results are retained, the already accepted remote object remains at each original conflicted path, and the competing local object is copied to `.sync-conflicts/<id>/local/<original-path>`. That synchronized hidden root is deliberately outside Vulcan's hidden-directory note scan and Obsidian's note index; it avoids beside-note duplicates while making the losing bytes inspectable on every eventual recipient. Structural objects, managed-root collisions, and recursive conflict-copy conflicts fail closed to refs and device-local artifacts. A reproducible two-parent conflict-record commit is anchored at the sibling `record` ref and carries the conflict ID, merge base, device, profile, policy, and immutable sources in trailers; when a safe materialization exists that commit anchors its exact tree rather than Git's marker-bearing conflict tree. Vulcan compare-and-swap publishes that provenance commit, then applies and verifies it through the ordinary Git-native worktree transaction before reporting the conflict; stale live refs or worktree drift restart or fail closed. Reports and durable records explicitly distinguish published and applied materializations. A pre-existing preservation ref that names any other commit is a hard failure rather than an overwrite. `vulcan-app` writes an immutable record and byte-preserving per-side artifacts beneath the device-local sync state directory, including materialization metadata, provenance ref, and revision. These device-local artifacts never appear in the materialized vault or Obsidian index, while the refs keep every original Git tree and creation provenance reachable independently of local state retention. Reviewed side and proposal resolutions keep the immutable original merge inputs, lease publication against the live provenance commit, parent the resolution from it, and atomically remove the hidden conflict-copy tree. Resolution commits similarly carry the conflict, chosen side, device, policy, and source trailers.
