...
- `vulcan sync conflicts [<id>]` is a read-only projection over that state for either the selected path or `--wiki <id>`. A preserved divergence is an immutable conflict session whose bounded path pages carry deterministic resolution-group identities: ordinary blob conflicts are independent, structural changes remain conservatively atomic, and whole-tree validation failures remain indivisible. Mutable per-batch records beside the session bind an exact group selection, expected accepted revision, output tree and commit, recovery witness, and prepared/published/applied/needs-rebase state without rewriting the evidence. Summaries expose total path/group and pending-group counts; detail exposes derived group progress. Callers can request `--path-offset` plus `--path-limit` for an explicitly bounded path response with total and next-offset metadata, and the companion capability contract advertises the same 256-path page and 128-group batch ceilings. Unpaged detail remains complete rather than silently truncated. Supersession is durable metadata beside the immutable record, never deletion or a fabricated resolution. Human list/detail output explains editor, local-side, and remote-side workflows with dry-run guidance. It does not reconstruct conflict state from `cache.db` or require the daemon.
