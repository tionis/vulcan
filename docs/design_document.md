...
- `vulcan sync retention-plan [<wiki>]` is the read-only boundary for that later workflow. It first requires the same exact accepted-ref and remote agreement as checkpoint creation, then uses typed ref enumeration and bounded first-parent traversal to report active-epoch pressure, retain the newest configured recovery checkpoints, classify only older recovery refs as expirable, and list semantic checkpoints as permanent. Planning never deletes refs or creates objects. In particular, it does not claim that deleting checkpoint refs prunes snapshot commits which remain ancestors of the canonical live tip; a real epoch rollover must be a separately reviewed lease-protected transaction that keeps offline-device reconciliation and archive expiry safe.
- `vulcan sync retention-apply [<wiki>]` currently applies only that separable local checkpoint-expiry decision. Its dry-run is the ordinary mutation-free plan. Mutating application recomputes under the same repository lock used by sync/checkpoint/resolution workflows and deletes each expirable recovery ref only when its exact planned object still matches; a moved ref fails closed and a partial or repeated run converges idempotently. The report fixes `epoch_rollover_applied` and `semantic_refs_changed` to false. This intentionally does not disguise local checkpoint cleanup as live-history rollover or remote archive expiry.
