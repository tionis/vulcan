...
- Configuration preserves that ownership boundary: `.vulcan/config.toml` may provide one complete `sync.merge_policy`, while `.vulcan/config.local.toml` may provide only `sync.merge_automation`. Shared automation ceilings and local policy replacements are diagnosed and ignored. The application layer resolves and validates the effective options before creating a transaction journal, so direct CLI, daemon, and companion execution use the same policy and malformed vault configuration cannot silently fall back during synchronization. An embedding caller's own review ceiling is combined restrictively and cannot be relaxed by config.
...
- Structured results carry explicit validation evidence. Before the merge commit or live-ref compare-and-swap, the backend has already enforced repository-path safety, parsed the selected syntax, checked Canvas and Bases root/entry shapes, kept a Markdown body's link-bearing surface byte-equivalent, rejected conflicted-file deletion, and reread the exact blob bytes and modes from the produced tree. Immediately before every bootstrap, fast-forward, or merge push, it also rebuilds the complete worktree tree and requires it to equal the captured candidate; intervening editor writes therefore cause a fresh finite-cycle attempt while the remote ref remains unchanged. A failed structured check returns to preserved-conflict handling. Whole-tree link analysis and configurable mass-deletion ceilings remain a separate application-level validation stage rather than being overstated by these per-path checks.
...
- Versioned deterministic synchronization merge policy (`[sync.merge_policy]`), including its ordered portable path/type rules. Device-local automation ceilings do not belong in this shared file.
