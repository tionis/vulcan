...
- Git-backed device synchronization (Roadmap 12.1–12.14) is implemented across three owners. `vulcan-sync` holds the synchronous backend contracts and the fixed-argument Git CLI engine: alternate-index capture that never touches the user's normal index, a versioned hidden-ref namespace, per-device remote safety heads, deterministic merge policy with built-in structured second-pass mergers (a closed, versioned merger registry and Git driver integration are planned under 12.19), immutable conflict preservation, and compare-and-swap publication. `vulcan-app` owns the finite sync transaction and everything that must survive interruption: device-local journals under the platform state directory, whole-tree link and mass-deletion validation, conflict sessions and their side/file/patch/editor/agent-proposal resolution workflows, checkpoints, retention, epoch rollover, and reviewable semantic history. `vulcan-daemon` only schedules and observes: it runs as `vulcan daemon start` or as a native per-user service, owns per-wiki watchers, periodic polling, remote wake-up listeners, the job ledger, notification sinks, the loopback companion transport, and opt-in semantic/conflict workers, and always invokes the same finite `vulcan-app` transaction that direct `vulcan sync run` uses. Daemon status is reconstructed from retained jobs, journals, and conflict records, never from `cache.db`.
- Two synchronization postures differ deliberately. Knowledge vaults are Vulcan-managed wikis: staged changes sync as ordinary worktree bytes through the alternate index, and only in-progress merge/rebase/cherry-pick/revert/bisect state or an unexpected HEAD change pauses a cycle. Files-only directories (10.9) may be arbitrary repositories under active development, so unattended files-only sync treats a staged index, detached HEAD, multiple worktrees, or nested repositories as evidence of a concurrent Git workflow and defers instead of capturing it.
- Shutdown sync is bounded: the daemon gives retained final-sync jobs 30 seconds before cooperative cancellation, and generated service definitions allow 40 seconds before a forced stop (`TimeoutStopSec=40s` for systemd, `ExitTimeOut` 40 for launchd). A forced stop is still safe because journals and the `apply.json` marker drive recovery on the next cycle.
- Distribution is archive-first and forge-neutral, with signed update channels, package-manager ownership of package-managed installs, and opt-in unattended update schedules that share the application-layer network/power policy.
- The full operational contracts for all of the above (job ledger, watcher and trigger coordination, companion authentication and protocol, service installation, distribution, ref layout, merge and validation evidence, conflict identity and resolution, retention, epochs, identity trailers, worktree application, semantic history, and optional workers) live in [the Git synchronization and daemon architecture specification](./specs/git-sync-architecture.md).
...
3. **Settings importer:** explicitly translates reviewed external settings into native Vulcan configuration. Import reports retain source and mapping provenance. New adapters are import-only: their source plugin files are never runtime authorities. A closed, pre-existing set of Obsidian core and plugin settings files is still read at runtime as lowest-precedence defaults (§14, *Legacy runtime defaults*); that set is frozen and may only shrink.
...
This is a deliberate split between two layers rather than two competing trust models. The device/key registry (`docs/specs/key-management.md`) is signed, linear Git history that answers *which device or signer made a change* and *which keys may be used for what*. Capability grants answer *what a subject may do in this vault* and are ordinary canonical objects validated by attenuation, not by signature. The two meet only at ingress: a governed policy may require that a change to the authorization namespace carries a signature that the registry verifies and maps to a subject whose grants cover the change. A registry signature never confers vault authority on its own, and a valid grant never needs to be signed.
...
- **Target chunk size** in characters. Default: 4000 (≈ 1000 tokens at the 4-characters-per-token estimate).
- **Overlap** (number of characters to repeat from the preceding chunk). Default: 0. Overlapping chunks can improve retrieval recall at the cost of increased embedding volume.
...
Before the main semantic pass, scan the raw source bytes for `%%` pairs (Obsidian comment delimiters) and record their byte ranges as comment regions in a sorted `Vec<Range<usize>>`. `%%` is only a delimiter in ordinary Markdown text: markers inside code spans, fenced or indented code blocks, math, and frontmatter are literal and must neither open nor close a comment. Otherwise a `LIKE '%%x'` in a code block pairs with a later real comment, hiding the prose in between from FTS and embeddings and indexing the private comment text instead. The scanner therefore first collects those literal ranges from a pulldown-cmark pass (skipped entirely when the source contains no `%%`) and pairs markers linearly outside them. A comment may still enclose a code block.
...
- On `MetadataBlock` event, capture raw YAML text. Parse with `serde_yaml` for the canonical layer. Preserve the raw text for lossless roundtrip. `serde_yaml` is archived and unmaintained upstream; `ParsedDocument::frontmatter` still exposes its `Value` type, so replacing it is a breaking change for library consumers. Replacing it is tracked as Roadmap 6.8 with frontmatter round-trip fixtures as the acceptance gate.
...
- Batch requests according to the provider's advertised limits. Use the blocking HTTP client with bounded concurrency (scoped worker threads, at most `max_concurrency` requests in flight) to avoid overwhelming local or rate-limited remote endpoints; `vulcan-core` and `vulcan-embed` stay synchronous, and only the daemon wraps them at an async boundary.
...
Many Obsidian vaults rely heavily on the Dataview community plugin for inline metadata and dynamic queries. Vulcan should support Dataview-style conventions as a first-class concern rather than treating them as opaque plugin syntax. The goal is a documented Dataview compatibility surface: DQL queries, inline fields, and implicit metadata that Dataview users rely on should produce equivalent results, and any construct Vulcan evaluates differently or not at all must surface as a diagnostic rather than silently different output. Consistent with the capability model in §4, exact upstream parity is a compatibility-profile promise, not a constraint on the native query model.
...
- Store inline fields in the existing `property_values` table with an `origin` column indicating extraction source. A single note may define the same property in both frontmatter and inline fields, and both rows are stored. Two surfaces then see them differently, by design: the merged page object used by DQL, DataviewJS, and inline expressions follows Dataview and collects every occurrence of a key into a list (see *Duplicate keys become lists* below); the typed relational projection used by `--where` filters, sorting, and Bases keeps the frontmatter value authoritative for that key's type, while inline occurrences remain queryable by origin.
...
| `file.ctime` | Date+Time | File creation (birth) time read from the local filesystem at query time, falling back to `mtime` where unavailable. Device-local: a clone or sync resets it, so portable workflows should use a `created` property |
...
**`file.day` resolution:** The first match wins: the note's date under the configured periodic-note conventions (§12b, Roadmap 9.16), then a filename matching `yyyy-mm-dd` or `yyyymmdd`, then a frontmatter `date` field (case-insensitive) with a valid date value. Returns `null` otherwise. Without periodic-note configuration this reduces to Dataview's filename-then-`date` rule.
...
DQL shares Vulcan's expression engine, `FilterExpression` lowering, `file.*` resolver, and cache tables with Bases, CLI flags, and JSON payloads, so there is one set of semantics for fields, functions, and comparisons. It does not compile into `QueryAst`: DQL's sequential data commands (repeated `WHERE`, `FLATTEN`, `GROUP BY` producing `rows`, TASK/CALENDAR modes, and `this`) do not fit the single-pass note query model, so DQL compiles to its own `CompiledDqlQuery` plan evaluated by `dql::eval`. Where a `WHERE` clause lowers cleanly to `FilterExpression`s it is pushed down to the shared filter path; everything else runs through the shared expression evaluator. Extending `QueryAst` to express the DQL pipeline remains the long-term convergence goal.
...
- Use the shared embedded QuickJS runtime ([rquickjs](https://github.com/DelSkayn/rquickjs)); §17c records why it was chosen over Boa and V8.
- Sandbox constraints follow the §17c tiers. DataviewJS blocks run at the configured default tier (`strict` unless changed): indexed vault reads such as `dv.pages()` and `dv.page()` work, raw file reads (`dv.io.load()`, `dv.io.csv()`, `dv.view()`) and vault writes need `fs`, network access needs `net`, and resource limits (timeout, memory, stack) apply at every tier except `none`. No `eval` of external scripts.
...
All five share the same filter and expression evaluation engine and run against the same cache tables. CLI flags, the JSON API, and Bases compile to `QueryAst`; DQL compiles to its own pipeline plan over the same engine (see *Recommended DQL implementation*); inline expressions are single expressions evaluated with a `this` binding, not queries. The difference is where the query is authored (CLI, API, file, or note body) and how results are rendered.
...
- **Templater (Roadmap 9.9):** Templater's `<% %>` tags contain JavaScript expressions, not Dataview expressions. Vulcan evaluates a native subset (literals and `tp.*` property paths and calls) in Rust and hands any other expression to the QuickJS runtime when `js_runtime` is compiled in; without it such tags produce a diagnostic. The DataviewJS sandbox (§12b DataviewJS) provides the JS runtime for Templater's `<%* %>` execution commands and `tp.web`/user script features. Native `tp.date`, `tp.file`, `tp.frontmatter` modules are implemented in Rust without the JS runtime.
...
- **Periodic notes (Roadmap 9.16):** Periodic note configuration supplies the first `file.day` source (§12b `file.*` namespace), so a note in a configured daily/weekly/monthly folder resolves to its period date even when its filename format is not ISO.
...
### Historical alternative: Go

*Kept for context; the Rust implementation is committed.*
...
- Link path interpretation and generation defaults (shortest-path, relative, or absolute) when no `.obsidian/app.json` is present; name and alias fallback always applies
...
If an `.obsidian` directory is present, the following files are read to provide Obsidian-compatible defaults. All others are ignored.

**Legacy runtime defaults.** Before the import-only rule in §4 was adopted, Vulcan already read a fixed set of settings files at every config load and applied them beneath `.vulcan/config.toml` and `.vulcan/config.local.toml`. That behavior is kept for compatibility, but the set is closed: `.obsidian/app.json`, `types.json`, `templates.json`, `daily-notes.json`, and the plugin `data.json` files for `periodic-notes`, `templater-obsidian`, `quickadd`, `dataview`, `obsidian-tasks-plugin`, `tasknotes`, and `obsidian-kanban`. These files are read-only inputs, only fill values the Vulcan config leaves unset, and report parse problems as config diagnostics. `vulcan config import` freezes their current values into `.vulcan/config.toml` so a vault can stop depending on them. No other plugin file may be added to this list; every new adapter (for example Folder Notes below) is import-only.
...
- **`.obsidian/app.json`** — Settings that affect link resolution and file handling:
  ...
  - `newLinkFormat`: `"shortest"`, `"relative"`, or `"absolute"` — how Obsidian *writes* new links. Vulcan uses it to generate link text during rewrites and to decide which exact interpretation a path-qualified link gets first (vault root for `absolute`, the source folder for `relative`). It does not narrow resolution: Obsidian resolves every link by path, then name, then alias regardless of this setting, so an exact-path miss in `absolute` or `relative` mode falls back to the shortest-path matcher.
...
- Treat the database as disposable; never require the user to preserve it across incompatible versions. The one expensive exception is embeddings, which cost provider time or money to recompute. Prefer additive migrations for vector tables so embeddings survive unrelated schema changes; when a breaking rebuild is unavoidable, call out the re-embedding cost in the release notes. Keying stored vectors by chunk content hash plus model identity, so that a rebuild could reuse them, is a candidate improvement rather than current behavior.
...
Commands are organized into a two-level hierarchy using clap nested subcommand enums. The 9.18 redesign was a pre-alpha clean break: the old flat command names were removed without aliases. Since then, a renamed command or tool may keep a hidden compatibility alias (for example `sync devices remove` for `prune-backup`, legacy MCP `daily_show`/`daily_list`, and the former tool-pack operation names). Such aliases are undocumented in help, map to exactly the new behavior, and are listed in the changelog.
...
| Level | Read indexed vault data | Read raw vault files | Write vault | Network | Resource limits |
|-------|-------------------------|----------------------|-------------|---------|-----------------|
| `strict` (default) | Yes | No | No | No | Yes |
| `fs` | Yes | Yes | Yes | No | Yes |
| `net` | Yes | Yes | Yes | Yes | Yes |
| `none` | Yes | Yes | Yes | Yes | No |

Indexed reads are cache-backed APIs such as `dv.pages()`, `vault.notes()`, and queries. Raw file reads (`dv.io.load()`, `dv.io.csv()`, `vault.note().read()`) require `fs`. Permission profiles still restrict paths, network origins, and host execution at every level; `docs/guide/sandbox.md` is the user-facing reference.
...
Phases 1–6 below are the original v1 plan and are complete; they are kept as the rationale for the core layering. Current status lives in `docs/ROADMAP.md`. Dependency edges are noted explicitly so that parallelizable work is visible.
...
- **Phase 10:** Multi-vault daemon with REST API — in progress. `vulcan daemon start/install`, the first vault-registry slice, sync supervision, and the companion transport exist; the REST API and full hosting consolidation (10.7) remain
- **Phase 11:** Git auto-versioning at the daemon level
- **Phase 12:** Device and file-tree synchronization through pluggable backends — the Git baseline (12.1–12.14) is implemented, with cryptographic identity, key management, synced secrets, deterministic artifact mergers, and selective materialization (12.15–12.20) as follow-ons; optional full-Space SilverBullet protocol support belongs here, while selective wiki exchange does not
...
- **CLI-to-tool parity** — **P1** — Every agent-facing tool that performs a vault operation maps to a CLI command with the same behavior and output format. MCP-only protocol and discovery tools (`capabilities`, `tool_packs`, and compound navigation tools such as `daily`, whose operations each map to a CLI subcommand) are the documented exceptions.
