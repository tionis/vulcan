...
- `vulcan sync conflicts [<id>]` is a read-only projection over that state for either the selected path or `--wiki <id>`. List output contains actionable unresolved records and is deliberately bounded to identity, paths, revisions, policy version, and resolution state; detail output returns the immutable full record and artifact metadata, including resolved or superseded history by ID. Every successful finite cycle marks older unresolved inputs as superseded, and a replacement conflict supersedes prior attempts for the same repository, so obsolete compare-and-swap inputs cannot keep daemon or direct status red forever. Supersession is durable metadata beside the immutable record, never deletion or a fabricated resolution. Human list/detail output explains editor, local-side, and remote-side workflows with dry-run guidance. It does not reconstruct conflict state from `cache.db` or require the daemon.
