...
- Unresolved Git merges receive deterministic conflict identities derived from the versioned policy hash, merge base, order-independent candidate tips, and sorted conflicted paths. Structured concurrent additions likewise order by immutable candidate object identity, so swapping local/remote roles or trigger arrival order cannot change a resolvable tree. No wall-clock timestamp, arrival position, or model confidence participates in winner selection. Before a conflict is surfaced, base/local/remote commits are anchored under create-only `refs/vulcan/conflicts/<id>/` refs. A reproducible two-parent conflict-record commit is anchored at the sibling `record` ref and carries the conflict ID, merge base, device, profile, policy, and immutable sources in trailers; a pre-existing preservation ref that names any other commit is a hard failure rather than an overwrite. `vulcan-app` then writes an immutable record and byte-preserving per-side artifacts beneath the device-local sync state directory, including the provenance ref and revision. These artifacts never appear in the materialized vault or Obsidian index, while the refs keep every original Git tree and creation provenance reachable independently of local state retention. Resolution commits similarly carry the conflict, chosen side, device, policy, and source trailers.
