...
- Each finite cycle also pulls the checked-out branch from its upstream before the hidden live refs move, using the user's own pull configuration (`pull.ff`, `pull.rebase`, `branch.<name>.rebase`) with `--no-edit` and no implicit autostash. Divergence past `pull.ff=only`, an interactive rebase request, or a merge/rebase conflict pauses only the branch lane with a reason while the file lane proceeds; a dirty worktree the pull must not overwrite defers to the next cycle. Repositories without an upstream, detached HEADs, and bare repositories skip with a reason. After a successful file lane, the branch tip is published to its upstream with the tracking ref observed during the attempt as an exact lease — never force-pushed; a moved remote reports for the next cycle and transport or policy failures record a detail instead of failing the converged file lane.
