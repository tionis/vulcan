...
### 4.7 Managed directories and selective materialization (planned)

Roadmap 10.9 and 12.20 extend the existing product in bounded slices. A managed vault is a local directory with explicit capabilities and policies; Git synchronization and Markdown knowledge services are independently optional. Preserve existing knowledge-vault defaults and unregistered direct CLI operation. Files-only management must not initialize a note index, invoke knowledge-specific validation, or enable scripts, semantic history, or agent conflict resolution merely because files have recognizable extensions.
