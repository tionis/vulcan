...
- Companion transports project a versioned synchronous service rather than calling Git directly. The service owns capability negotiation, wiki lookup, reconstructed status, credential-scoped idempotent sync/resume enqueueing, pause, conflict inspection and deterministic resolution, semantic-plan creation, and job status/cancellation. Repository-reading and mutating calls reapply the registration's named permission profile. Capabilities only advertise operations and agent modes with working implementations; HTTP and WebSocket adapters add their transport capabilities without duplicating these workflows.
