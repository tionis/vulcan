...
- The Git ref layout is a formal versioned contract owned by `vulcan-sync`, not a collection of call-site strings. Version 1 centralizes validated builders for device-local candidate/fetched/pending refs, epoch archives, conflicts and their recovery/decisions, checkpoints, semantic proposals, and detached-loss recovery. Sync reports expose `refs.namespace_version`, and created live and epoch commits carry `Vulcan-Ref-Namespace` trailers so recovery tooling can interpret reachable state independently of the rebuildable cache. Local `refs/vulcan/**` are never published. Generic Git transport conformance proves custom refs can use fetch and leased mutation, but the interoperable remote default remains the hidden branch `refs/heads/__vulcan-sync/live` with its epoch subtree until a deployed Forgejo passes explicit permission, advertisement, webhook, maintenance, and Actions checks.
...
- Normal-index and Git-operation safety gates protect reconciliation and application without sacrificing recoverability. Staged changes sync as ordinary worktree bytes through the alternate index while the normal index is never staged, reset, or rewritten; only merge/rebase/cherry-pick/revert/bisect state or an unexpected HEAD commit or symbolic-ref change pauses the cycle. When such a state is present, Vulcan first captures the worktree to its local candidate ref and fetches an existing remote live tip, then emits a structured pause reason and retains the paused transaction journal. It does not push, apply, reset the normal index, or reinterpret a branch switch. A safety change detected after remote acceptance anchors that accepted commit under the pending ref before pausing application.
- Each finite cycle also fetches and, when possible, pulls the checked-out branch from its upstream before the hidden live refs move, using the user's own pull configuration (`pull.ff`, `pull.rebase`, `branch.<name>.rebase`) with `--no-edit` and no implicit autostash. Divergence past `pull.ff=only`, an interactive rebase request, or a merge/rebase conflict pauses only the branch lane with a reason while the file lane proceeds; a dirty worktree the pull must not overwrite defers to the next cycle. Repositories without an upstream, detached HEADs, and bare repositories skip with a reason. Immediately after a healthy branch pull lane, the branch tip is published to its upstream with the tracking ref observed during the attempt as an exact lease — never force-pushed — before conflict-prone hidden file reconciliation begins. Ordinary committed history therefore continues to fetch, pull, and push even when the file lane later preserves an unresolved conflict. A moved remote reports for the next cycle, while transport or policy failures remain explicit branch-lane failures without preventing the file lane from proceeding.
