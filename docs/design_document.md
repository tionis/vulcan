...
- `sync status` is a read-only preview, not a retained daemon-job status or a completed finite cycle. It queries remote refs, previews the checked-out branch, compares the worktree with its last local snapshot, and classifies the hidden file lane as up to date, locally changed, remotely different, changed on both sides, missing on one side, or uninitialized. It does not fetch remote objects, merge, apply, or claim that differing trees are conflict-free. Human output labels the operation and its successful items as a preview/inspection, states that no changes were applied, and must not render the internal `planned` outcome as though synchronization itself succeeded.
