...
- `sync status` is a read-only preview, not a retained daemon-job status or a completed finite cycle. It may query remote refs and preview the checked-out branch, but it does not fetch, merge, apply, or reconcile the hidden file lane. Human output labels the operation and its successful items as a preview/inspection, states that no changes were applied, and must not render the internal `planned` outcome as though synchronization itself succeeded.
