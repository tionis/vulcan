...
Last architecture/status reconciliation: 30 August 2026
...
- **Cryptographic key management:** The keychain is a byte-custody backend. `KeyManager` in `docs/specs/key-management.md` uses typed operations, exact usages, separate sensitive-role keys, and intersected caller/local/registry/resource permissions. A registry starts with one owner; any enrolled administrator can sign a change using ordinary Git SSH signatures and linear parent-authorized history. Devices use last accepted permissions offline without automatic expiry, and enforce revocations when updates arrive. Disconnected replicas retain pending changes for explicit reconciliation and publication. Consumer adapters must preserve these semantics and resource restrictions; existing authentication and updater trust remain independent.
- **Optional synced secrets:** The accepted follow-on in `docs/specs/key-management.md` and Roadmap 12.18 uses one dedicated native age recipient per secret, reusable across secrets. Sync carries versioned ciphertext records and public key references; private age identities are shared out of band and imported into device-local custody. Reads work offline, and missing keys leave records locked without blocking sync. Secret-access registries, automatic distribution, recipient management, and rotation workflows are deferred. Sharing a key grants access to all accessible versions encrypted for it; later registry removal cannot undo that access.
- **Local data directory:** `.vulcan/` in the vault root, containing `cache.db` (SQLite cache on desktop), `config.toml` (shared vault configuration), optional `config.local.toml` (device-local overrides), and non-cache durable workflow state where required. On Android, the rebuildable SQLite cache, vault write lock, mdbase transaction state, and integration/Outline state live in per-vault Termux state so WAL, file locking, and state-directory sync do not depend on shared storage. Canonical Markdown remains in the shared worktree; mdbase writes fail before mutation if that worktree cannot sync directories. Direct commands are vault-scoped; the planned daemon adds a separate user-level multi-vault registry without making it canonical content storage.
...
- **Managed-directory direction (planned, Roadmap 10.9 and 12.20):** Keep knowledge bases central while reusing directory management and file synchronization for document/media collections and other repositories. Markdown indexing is an optional capability. Separate the logical canonical file tree from each device's selected materialization; absence outside that selection is never a deletion. Keep this in the existing project until concrete reuse justifies extraction.
...
The planned selective-materialization model qualifies completeness, not file authority: local files remain authoritative for local edits, but a partial device's directory listing is not the complete vault inventory. The accepted Git tree plus verified local changes represent logical contents; durable device-local selection and recovery state explain which paths are materialized. SQLite is never authoritative for either content or selection. A cache rebuild covers the locally available knowledge subset and reports incomplete coverage without downloading the rest implicitly.
...
- The migration from today's daemon and CLI-owned listeners to one reusable host follows the
  [unified hosting contract and inventory](./specs/unified-hosting.md). Resident services come only
  from device configuration; foreground hosts remain invocation-local; direct finite commands and
  MCP stdio remain daemon-independent. The contract fixes service identity, lifecycle, routing,
  ownership, execution context, shutdown, and compatibility before transports are moved.
...
- Stable enough for reuse: serializable request/report structs in `vulcan-core` and `vulcan-app`, cache/query/search/graph/task/note/template/export/config services, permission-profile resolution, skill command registry metadata, and permission-aware MCP catalog semantics. Phase 10 daemon REST/API work should depend on `vulcan-core` and `vulcan-app`; MCP transport reuse should either promote the existing split `vulcan-cli::mcp` internals into a small support crate or keep MCP as a CLI-only endpoint until that reuse boundary is proven.
...
- Profiles may instead define a shared additive selection plan. Each query or graph clause produces a document set, all clause results are unioned and deduplicated by resolved document identity, and profile-wide exclusions and permission filters always win. Multiple seeds may share one graph clause when their traversal settings match; separate clauses allow each seed or seed group to use its own direction, depth, and filters.
- Graph selection distinguishes `result_query`, which filters reached notes from the output without stopping traversal, from `traverse_query`, which controls whether traversal may continue through a reached note. Seeds have an explicit `include_seeds` policy. Traversal is cycle-safe, deterministic, bounded by a configurable node limit, and records clause/seed/depth provenance for plans and machine-readable export artifacts.
- Graph direction is explicit (`outgoing`, `incoming`, or `both`). Global exclusions and permission boundaries are hard traversal boundaries: selection must neither include nor traverse through excluded or unreadable notes.
...
- The first useful transform families are structural stripping rules such as excluded callout classes and excluded heading sections, metadata stripping rules such as excluded frontmatter keys and excluded inline fields, and ordered literal/regex replacement rules for publication redaction or normalization without mutating the source vault.
...
- Export/render pipelines that consume transformed notes must rebuild derived metadata such as frontmatter JSON, merged properties, resolved links, copied attachments, and inline-expression evaluation from the transformed content so stripped data cannot leak through cached projections.
...
- External Markdown connectors may add a typed, direction-aware compatibility pass after profile transforms. The Outline boundary strips outbound frontmatter, converts Obsidian callouts to Outline container fences, translates planned document links to durable remote IDs for API publication, and offers opt-in generated-TOC removal. Reversible callout and document-link translations must also be available to inbound routes; irreversible stripping and attachment download/materialization remain explicitly directional.
- Connector-specific unsupported-link fallbacks may expose a typed, synchronous JavaScript callback when declarative policies cannot preserve enough intent. Such callbacks run only for explicitly trusted vaults in a pure resource-bounded runtime with structured input/output, deterministic time/random guards, located failures, and source-path/content-hash provenance in dry-run and apply reports. They must not gain vault, filesystem, network, shell, plugin, or tool APIs, change selection, or read excluded content. Generic audience-filtering transforms remain declarative so redaction and exclusion rules stay statically inspectable.
...
Vulcan's multi-user authorization model is capability-first and vault-canonical. Typed wiki objects represent identities, groups, root grants, delegated grants, revocations, policy ceilings, role templates, public credential metadata, and durable audit records. A trusted bootstrap workflow creates the first root grant; after that, holders may issue child grants only when they possess delegation authority, and every child must be a provable attenuation of one parent. A credential that needs authority from several independent parents carries several independently derived grants rather than merging their provenance.

The monotonic attenuation model is inspired by [macaroon caveats and user-derived least-privilege tokens](https://fly.io/blog/macaroons-escalated-quickly/), while deliberately separating authorization semantics from token encoding. Vulcan does not require grant signatures or an external cryptographic trust anchor: direct filesystem or repository control is already outside the web/API authorization boundary and exposes the plaintext vault.

```text
canonical authorization wiki objects
          |
          v
...
Authorization objects live under a configured reserved namespace such as `System/Authorization/`. They are first-class wiki content for links, history, queries, and administration views, but they are control-plane objects rather than ordinary notes. Generic note writes, refactors, templates, plugins, sync writes, and agent tools cannot create, change, move, or delete anything in that namespace. The namespace setting itself is security-critical control-plane configuration and receives the same mutation and ingress protection. Dedicated `auth` workflows validate identity-management authority, parent lineage, attenuation, and the resulting complete graph before applying atomic file mutations. Invalid, duplicate, cyclic, or unsupported authorization objects produce diagnostics and fail closed; the SQLite projection remains fully rebuildable from the canonical files.
...
Document extraction is an upstream producer concern rather than a Vulcan parser backend. The Markdown Artifact Format (MDAF) provides the stable boundary: one primary Markdown document and its assets, normalized source-selector and optional outline sidecars, complete opaque producer renditions, and an activity-level provenance graph travel together as an immutable directory or ZIP artifact. Sources may be PDFs, images, audiovisual media, web content, office documents, structured data, text, compound media, or future formats; Markdown names the normalized output rather than constraining the input. Locators compose ordered intervals, spatial regions, grid ranges, text quotes, standard fragments, and namespaced extensions. Vulcan validates and imports this neutral contract without running extraction or branching on source media types, extractor identities, native block types, page-anchor conventions, or asset filenames.

The normalized core stays deliberately small. UTF-8 Markdown byte spans map to ordered source-coordinate intervals whose unit is an open string, so pages, slides, sheets, frames, timestamps, and source lines share one consumer model. A separately aligned outline remains an alternative authority rather than being silently merged with Markdown headings. Import defaults select a validated aligned outline when it can represent the requested projection and otherwise use Markdown headings with a diagnostic; callers can force either authority, and reports identify the authority actually used. Bounding boxes, tables, confidence payloads, recursive block trees, and other extractor-native evidence remain lossless declared renditions or namespaced extensions until a cross-producer consumer requirement justifies another normalized capability.
...
Portable exchange is split by semantic unit instead of forcing documents, evidence artifacts, and complete wikis into one container. TextBundle and its compressed TextPack representation exchange one mutable plain-text document with referenced assets and application-specific metadata. Vulcan preserves unknown metadata on inspection and validation, imports a TextPack into an explicit collision-free vault destination, and exports one canonical Markdown note plus its referenced local assets. TextPack is an interoperability adapter, not an evidence or provenance format; an extraction that must retain source coordinates, native responses, and reproducible transformation history continues to use MDAF.

A Markdown Wiki Package exchanges a snapshot of multiple Markdown notes and assets. Its semantic model is independent of serialization: an unpacked `.wikibundle` directory and a compressed `.wikipack` ZIP expose the same `wiki.json` manifest and `content/` member tree and have the same BLAKE3 logical identity. The manifest records only portable authority that cannot be safely inferred from bytes alone: format and schema version, title, Markdown dialect/features, declared member paths and roles, content digests, optional stable document IDs, explicit folder-note/navigation conventions, producer information, immutable parent identities, and namespaced extensions. Parsed links, backlinks, properties, FTS rows, embeddings, resolved identities, and other indexes remain derived and are never required exchange state.
...
The convenience profile is not the persisted file format. In particular, authoritative shared state and a fast local query representation may deliberately differ. A replicated or event-sourced store identifies its accepted canonical frontier from synchronized immutable files, then maintains a host-owned local materialization for reads, indexes, and transactions. That materialization is derived even when the logical dataset is authoritative: it is safe to discard only because the accepted snapshot/checkpoint plus retained operations can reconstruct it. Its cache key includes the store identity, canonical frontier, schema version, adapter version, reducer/projection identity, and relevant configuration. Vulcan validates and advances it incrementally when possible and rebuilds it when any key or integrity witness changes. Apps never mark an unreconstructable database authoritative merely by calling it a cache.

Planned canonical persistence shapes beyond stable App v1 are:

| Shape | Canonical synchronized representation | Local materialization | Best fit | Merge model |
| --- | --- | --- | --- | --- |
| vault collection | Markdown/mdbase records plus schemas/contracts | rebuildable shared SQLite indexes | human-readable, linkable wiki knowledge | file and record-aware three-way merge |
| atomic artifact | one closed SQLite database, media object, or other complete file | optional read snapshot/index | specialized single-writer or explicitly serialized data | byte identity or required review |
| immutable event set | versioned manifest plus immutable stable-ID events, optional packs, and checkpoints | reducer-specific KV/SQLite projection at an exact frontier | audit trails, workflows, activity/history, event-sourced domains | path union or validated stable-ID set union |
| replicated relational change set | versioned schema/checkpoint identity plus immutable replica/sequence change batches | SQLite/cr-sqlite or another reviewed replica adapter | relational queries and offline multi-master updates | validated batch-set union, then deterministic adapter application |
| Automerge document | one pinned Automerge state/change-set representation per bounded logical document | loaded Automerge state plus optional app query projection | small document-shaped maps/lists/text needing offline concurrent edits | Automerge state join plus canonical serialization |
| content-addressed blob set | immutable digest-named objects plus typed references | local deduplicated blob cache | media and large opaque values | identity/path union; collisions are corruption |
| live session | daemon-owned expiring state and optional recovery checkpoints | in-memory/session database | presence, cursors, queues, timers | protocol convergence, not file-tree merge |

Canonical special-format stores live in a host-managed, vault-contained data allocation bound to the instance and store. They participate in history, backup, permission inspection, and Phase 12 file-tree synchronization, while note scanning, Obsidian presentation, and ordinary publication exclude them unless an explicit adapter projects user-visible content. Private device stores and derived materializations remain in excluded host-owned allocations. The concrete canonical root and filename extensions are part of each future format specification, not conventions an app may invent; this avoids generic extension claims and lets sync select mergers from trusted store bindings rather than attacker-controlled filenames.
...
The [MDB performance and native integration contract](specs/mdb/PERFORMANCE_AND_NATIVE_INTEGRATION.md) defines focused native execution beneath the portable mdbase model for both Apps and standalone scripts. Its warm indexed metadata-query target is p95 below 50 ms and p99 below 100 ms at the local host boundary, with direct-CLI, freshness, scale, and correctness gates specified separately. Native integration reuses shared core/app services and the existing query representation; it does not introduce a second collection format or assume limited future mdbase adoption. First-party App-backend readiness requires these gates, while direct CLI/script work remains independent of the full App platform.
...
Three distinct formats sit behind that general contract and must not be collapsed into “a synced database”:

- An **immutable event set** stores application/domain facts with stable event identity, schema, causal or actor/sequence metadata, payload digest, and optional signature. One event per immutable path is the safest Git representation; bounded canonical packs reduce small-file overhead. Reducers and projections run only after sync accepts the canonical files, never inside a merge driver. Concurrent set union guarantees retention of events, not correctness of arbitrary business semantics: a format must declare whether concurrent events commute, have causal prerequisites, or require an application-visible semantic conflict.
- A **replicated relational change set** stores adapter changes rather than mutable database pages. Immutable batches bind replica, sequence/range, schema, source checkpoint/frontier, payload digest, and adapter format. A local database applies newly accepted batches transactionally and records its exact frontier. Canonical checkpoints bound bootstrap and replay; deleting history before every required replica or retention policy acknowledges that checkpoint is explicit compaction, not ordinary garbage collection.
- An **Automerge document store** holds bounded application documents whose maps, lists, text, and scalar changes benefit from CRDT semantics without a relational engine or application-defined event reducer. It is separate from Phase 14's device-local editor crash recovery. A canonical profile requires a pinned Automerge format, actor/identity policy, schema envelope, size/change-count limits, attachment references, compaction/history policy, and deterministic serialization or content identity. One large document must not become the default container for unrelated application records.

Event and changeset formats may share an outer immutable-record envelope and storage/packing machinery, but their semantics differ: event reducers interpret domain facts, while database adapters apply typed row changes. Automerge changes retain their library-defined causal semantics. Each store exposes accepted frontier, materialization frontier, lag, rebuild/repair status, retained bytes, and compaction eligibility through common administration reports. A checkpoint is merely an acceleration artifact while older canonical operations remain; it becomes authoritative only through an explicit, journaled compaction plan that proves the declared recovery and offline-replica horizon remains satisfied.
...
### 4.7 Managed directories and selective materialization

Roadmap 10.9 ships device-local `knowledge` and `files-only` profiles for registered directories: knowledge remains the backward-compatible default, while files-only sync skips Markdown indexing and knowledge-specific validation/workers. The registry records a versioned capability contract and a separate full-tree materialization policy; directories may be registered without Git synchronization. Files-only devices retain concurrent automatic merges for review to avoid accepting bytes that a knowledge device would reject. Selective materialization remains planned under 12.20. A managed vault is a local directory with explicit capabilities and policies; Git synchronization and Markdown knowledge services are independently optional. Preserve unregistered direct CLI operation. Files-only management must not initialize a note index, invoke knowledge-specific validation, or enable scripts, semantic history, or agent conflict resolution merely because files have recognizable extensions.

Maintain three ownership boundaries: `vulcan-daemon` owns registration, observation, scheduling, and service lifecycle; dependency-light synchronous `vulcan-sync` owns file-tree capture, reconciliation, transport, and recovery; `vulcan-app` composes finite workflows and optional `vulcan-core` content services. Shared content-aware merge policy remains deterministic across devices; choosing a local profile must not silently change accepted merge semantics. No second daemon, mandatory plugin framework, or separate product is required.

Sparse checkout selects working-tree paths, partial clone selects downloaded Git objects, shallow history limits ancestry, and LFS selection controls separate large-file payloads. Keep these independent in typed policy and capability reporting. Device-local selection, offline pins, download budgets, and eviction preferences must not travel as another device's desired state. Shared repository policy still governs canonical content, attributes, merge rules, and required control metadata. Until each combination passes conformance, detect it and refuse unsafe mutation with actionable diagnostics.

Capture overlays verified local changes on a known complete logical tree while preserving excluded entries. Worktree application verifies the exact selected projection and policy generation under the same transaction lock; changing selection, hydrating content, or evicting a local copy never implies canonical deletion. Preserve dirty/unpublished bytes, normal-index state, untracked collisions, and recovery evidence across interruption. Missing promised objects, intentionally absent paths, unhydrated LFS pointers, actual deletions, and corruption require distinct handling.

Enforce network and resource policy at Git/helper invocation boundaries, including implicit lazy fetches and LFS smudge operations. Status and doctor remain offline; previews report required downloads without performing them. Fetch only needed objects within explicit limits for mutation or conflict resolution, and stop recoverably when required evidence is unavailable. Streaming and bounded concurrency are required for media payloads; resource-limit failures must not fall back to full downloads. Keep metadata convergence, selected-content availability, pending publication, and offline readiness separate in reports.

Knowledge services operate on a coherent selected subset. Link diagnostics, search, exports, and publication must distinguish unavailable content from missing content and report coverage; operations requiring omitted bytes need an explicit materialization step. This does not make `vulcan-core` remote-aware or require virtual filesystems. External document routes still pass through inspected local content rather than relaying between remotes.

File-tree synchronization is distinct from development-branch management and complete backup. Unattended files-only synchronization conservatively rejects detached HEAD, staged changes, multiple worktrees, in-progress Git operations, and nested repositories; it rechecks the selected branch before applying changes. External Git clients do not share Vulcan's lock, so active development checkouts with concurrent Git activity remain unsupported. Complete backup requires explicit ref/history/LFS retention and restore verification; metadata convergence alone is not a backup guarantee. Initial scope is files-only directories, sparse correctness, selective LFS, then blob-filtered partial clone; shallow ancestry support is separately gated.
...
- `authorization_objects`, `grant_lineage`, and `effective_grants`: rebuildable Phase 17 projections of canonical identity, group, grant, revocation, policy, credential-metadata, and audit objects under the reserved authorization namespace. Secret verifiers, sessions, OAuth state, and runtime counters never enter these cache tables.
...
- Generate Bash, Fish, and Zsh completions with runtime candidates for vault resources and configured identifiers. Any command that lists a reusable named object—such as a publication profile, export profile, route, permission profile, saved report, template, plugin, alias, or skill—should also complete that identifier wherever the CLI accepts it, including option values.
...
Credential profiles may reference static header/cookie secrets through the versioned device-local `SecretStore` in `docs/specs/device-key-custody.md` or an environment variable; they must not invent provider-specific locators or automatic fallback. A device-local browser-cookie source may additionally name a supported browser and profile, but using it is a distinct high-risk permission: it is opt-in, origin/domain-scoped, and never an automatic fallback after a public request fails. Browser cookie values are decrypted only for the operation, filtered using browser-equivalent host/path/secure/expiry rules plus the configured allowlist, redacted from all output, and never written to notes, shared config, durable workflow state, cache rows, diagnostics, or logs. Profile selectors and structured secret references may live in `config.local.toml`; cookie values may not.
