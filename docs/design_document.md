...
- Unresolved Git merges receive deterministic conflict identities derived from the versioned policy hash, merge base, order-independent candidate tips, and sorted conflicted paths. Structured concurrent additions likewise order by immutable candidate object identity, so swapping local/remote roles or trigger arrival order cannot change a resolvable tree. No wall-clock timestamp, arrival position, or model confidence participates in winner selection. Before a conflict is surfaced, base/local/remote commits are anchored under create-only `refs/vulcan/conflicts/<id>/` refs. For bounded regular-file conflicts with a unique base, Vulcan also constructs a safe materialization candidate: normal clean merge results are retained, the already accepted remote object remains at each original conflicted path, and the competing local object is copied to `.sync-conflicts/<id>/local/<original-path>`. That synchronized hidden root is deliberately outside Vulcan's hidden-directory note scan and Obsidian's note index; it avoids beside-note duplicates while making the losing bytes inspectable on every eventual recipient. Structural objects, managed-root collisions, and recursive conflict-copy conflicts fail closed to refs and device-local artifacts. The candidate is initially Git-object-only so publishing and worktree application can gain their own stale-input, platform, cleanup, and resolution-lease transaction. A reproducible two-parent conflict-record commit is anchored at the sibling `record` ref and carries the conflict ID, merge base, device, profile, policy, and immutable sources in trailers; when a safe candidate exists that commit anchors its tree rather than Git's marker-bearing conflict tree. A pre-existing preservation ref that names any other commit is a hard failure rather than an overwrite. `vulcan-app` writes an immutable record and byte-preserving per-side artifacts beneath the device-local sync state directory, including the candidate metadata, provenance ref, and revision. These device-local artifacts never appear in the materialized vault or Obsidian index, while the refs keep every original Git tree and creation provenance reachable independently of local state retention. Resolution commits similarly carry the conflict, chosen side, device, policy, and source trailers.
