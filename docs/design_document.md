...
File-tree synchronization is distinct from development-branch management and complete backup. Active development checkouts need a separately tested contract for branch switches, staging, worktrees, rebases, and concurrent Git operations before unattended management is enabled. Complete backup requires explicit ref/history/LFS retention and restore verification; metadata convergence alone is not a backup guarantee. Initial scope is files-only directories, sparse correctness, selective LFS, then blob-filtered partial clone; shallow ancestry support is separately gated.
