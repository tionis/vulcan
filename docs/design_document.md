...
- `vulcan daemon install/uninstall` projects that same foreground process into a native per-user service without introducing a second lifecycle implementation. Linux uses an atomically replaced `systemd --user` unit with restart-on-failure, Windows uses a limited per-user Task Scheduler logon task, and the planned macOS projection uses a per-user LaunchAgent managed through explicit `launchctl` argument vectors. Planning is explicit and mutation-free under `--dry-run`, service definitions contain no secret values, symlinked definitions fail closed, and uninstall preserves all Vulcan registry, credential, journal, conflict, and vault state. A permission-checked device-local `daemon.env` is resolved by a platform-neutral startup path where the service manager cannot safely provide an environment file; generated plists or task definitions never embed expanded credentials.
...
- Desktop distribution is archive-first and forge-neutral. Versioned per-target Linux, Windows, and macOS archives, checksums, and a machine-readable artifact manifest are the canonical release contract; installer scripts and package-manager definitions consume those artifacts rather than rebuilding Vulcan's configuration or service semantics. Installation never registers a wiki or enables the daemon implicitly, and removal preserves device and vault state. Homebrew may expose its own explicit service integration using a stable package path, while the built-in service installer remains available for direct archives. Git stays a separately diagnosed runtime dependency for the initial synchronization backend.
