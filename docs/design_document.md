...
- Unresolved Git merges receive deterministic conflict identities derived from the versioned policy hash, merge base, order-independent candidate tips, and sorted conflicted paths. Before a conflict is surfaced, base/local/remote commits are anchored under `refs/vulcan/conflicts/<id>/`; `vulcan-app` then writes an immutable record and byte-preserving per-side artifacts beneath the device-local sync state directory. These artifacts never appear in the materialized vault or Obsidian index, while the refs keep every original Git tree reachable independently of local state retention.
