...
- Remote device safety heads are operator-visible recovery state, not automatic merge inputs. `vulcan sync devices list` enumerates exact heads, `devices fetch <id>` anchors a selected head and current live under local recovery refs and reports `same`, `integrated`, `contains_live`, or `diverged`, plus changed paths and isolated-worktree guidance. `devices remove <id>` cannot remove the current device, requires those exact fetched recovery refs, rechecks the remote objects, and deletes with an exact lease only when the candidate is an ancestor of live or has the same tree. Divergent or live-ahead device information fails closed; remote deletion deliberately retains the local recovery ref. This gives another node the tools to resolve a lost device's conflicts without silently mixing its files into canonical live.
