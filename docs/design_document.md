...
- Configuration preserves that ownership boundary: `.vulcan/config.toml` may provide one complete `sync.merge_policy` plus shared `sync.tree_validation` ceilings, while `.vulcan/config.local.toml` may provide only `sync.merge_automation`. Shared automation ceilings and local policy or whole-tree-limit replacements are diagnosed and ignored. The application layer resolves and validates the effective options before creating a transaction journal, so direct CLI, daemon, and companion execution use the same policy and malformed vault configuration cannot silently fall back during synchronization. An embedding caller's own review ceiling is combined restrictively and cannot be relaxed by config.
