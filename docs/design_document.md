...
- MCP support should stay inside `vulcan-cli` for Phase 9.29 while being split internally into transport-neutral registry/catalog/handler pieces plus transport-specific stdio and Streamable HTTP adapters. A dedicated `vulcan-mcp` crate is intentionally deferred until Phase 10's daemon integration proves the exact reuse boundary. Permission profiles remain the common authorization model for CLI, MCP, daemon, and assistant-facing entrypoints.
