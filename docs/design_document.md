...
- Companion transports project a versioned synchronous service rather than calling Git directly. The service owns capability negotiation, wiki lookup, reconstructed status, credential-scoped idempotent single-wiki and aggregate selection enqueueing, pause/resume, conflict inspection and deterministic resolution, optional agent proposal creation plus explicit proposal approval/rejection, semantic-plan creation, and child/aggregate job status and cancellation. Repository-reading and mutating calls reapply the registration's named permission profile. Resolution providers, endpoints, and credentials are configured server-side rather than accepted from a companion request; the provider reports its actual network endpoint for permission checks, and proposal support is advertised only while one is configured. A configured daemon claims each repository/conflict pair before calling the provider, permits one active proposal request for that pair, fails concurrent requests immediately, and releases the claim through scoped cleanup; capability negotiation reports both the limit and its `daemon_process` scope so clients do not mistake this device-local guard for cross-device coordination. HTTP and WebSocket adapters add their transport capabilities without duplicating these workflows.
