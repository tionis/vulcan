...
- `vulcan daemon install/uninstall` projects that same foreground process into a native per-user service without introducing a second lifecycle implementation. Linux uses an atomically replaced `systemd --user` unit with restart-on-failure and an optional device-local `daemon.env`; Windows uses a limited per-user Task Scheduler logon task. Planning is explicit and mutation-free under `--dry-run`, generated commands are argument-vector based rather than shell strings, service definitions contain no secret values, symlinked definitions fail closed, and uninstall preserves all Vulcan registry, credential, journal, conflict, and vault state.
