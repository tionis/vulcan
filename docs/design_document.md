...
- The read-only device inventory reports remote safety heads, whether each has a matching local recovery ref, retained local recovery refs with no remote head, and names without any backup. A local recovery copy marked current only matches the observed remote revision; it does not establish integration with live or permission to delete the backup.
