...
- `vulcan sync conflicts [<id>]` is a read-only projection over that state for either the selected path or `--wiki <id>`. List output contains unresolved records and is deliberately bounded to identity, paths, revisions, policy version, and resolution state; detail output returns the immutable full record and artifact metadata, including resolved history by ID. It does not reconstruct conflict state from `cache.db` or require the daemon.
