...
- The initial companion transport refuses non-loopback listeners. HTTP operations use JSON, `Authorization: Bearer`, `Vulcan-Protocol-Version: 1`, exact Origin checks when the header is present, and authenticated CORS behavior; manual sync and resume additionally require `Idempotency-Key` and return HTTP 202 with the retained job report. WebSocket upgrades negotiate `vulcan.v1` and authenticate a second `vulcan.bearer.<token>` subprotocol, avoiding URL credentials and browser-inaccessible custom headers. The bounded event loop sends an initial snapshot and then only changed snapshots of registrations, reconstructed per-wiki status, and retained jobs. Synchronous app/registry/supervisor work runs behind the async boundary with `spawn_blocking`.
