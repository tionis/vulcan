...
Registered directories can use a `knowledge` or `files-only` profile, configured with `vulcan vault add <id> <path> --profile files-only` or `vulcan vault set <id> --profile files-only`. `knowledge` is the default and keeps the existing knowledge-vault workflow. `files-only` runs the same Git file synchronization without initializing or refreshing a Markdown index, or running knowledge-specific tree validation, scripts, semantic history, or agent conflict resolution. Concurrent automatic merges are retained for review on files-only devices so a local profile cannot accept bytes that a knowledge device would reject. The profile is device-local registration metadata; it is not written into the vault's `.vulcan/config.toml`.

Selective materialization remains planned in [Roadmap 12.20](../ROADMAP.md#1220-selective-materialization-and-large-repository-synchronization). Sparse checkout, partial clone, shallow-history reconciliation, and selective LFS hydration are not currently advertised as supported sync combinations. Existing Git/LFS filter checks do not establish those guarantees. The future model preserves excluded paths and reports metadata synchronization separately from local payload availability; this guide will gain tested setup and recovery recipes as those slices ship. See [Roadmap 10.9](../ROADMAP.md#109-managed-directory-capabilities-and-knowledge-profiles) for the remaining managed-directory work.
