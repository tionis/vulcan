...
Published search deliberately stays smaller than the vault search engine. The important user-facing
constraints are:
...
The implementation design for richer shared search belongs in the roadmap and design document. For
current deployments, treat the generated client-side index as the complete supported contract and
do not assume that CLI search operators will work in the website search box.
