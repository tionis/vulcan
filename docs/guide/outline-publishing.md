...
Automatically allocated document UUIDs are scoped by publication profile and collection, so selecting one local note through multiple profiles does not claim a document created for another collection. Create state is saved as pending before the API mutation and finalized only after a `documents.info` readback confirms the returned server ID, active collection membership, and parent. Explicit pull adoption remains the only automatic route to an existing ID and validates the same ownership boundary. If legacy or interrupted state points into another collection or at an archived/deleted document, Vulcan can prove that the ID never represented an established document in the configured target and plans a fresh create without requiring manual mapping edits. A missing ID that cannot be proven foreign remains a conflict.
