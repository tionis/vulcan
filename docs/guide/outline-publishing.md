...
Pulls default to a 10,000-document total-work ceiling, configurable with `--max-documents`. The limit is checked before planning or mutation. Outline's offset pagination does not provide a true collection snapshot cursor, so Vulcan verifies that the advertised total remains stable, rejects duplicate documents, and requires the final item count to match; retry when a collection changes during listing.
