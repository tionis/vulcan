...
posts a high-priority notification for non-network failures and applies the configured network
failure notification policy to transient network errors. A later successful run removes it. If a daemon is deliberately kept running, enabling
