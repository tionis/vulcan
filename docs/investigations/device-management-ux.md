...
| P1 | The per-vault backup list and installation-wide registered-wiki view retain local recovery evidence when remote observation fails. Explicit offline mode skips remote Git observation and reports `not_requested`. The global view still uses the default Git target. | Add explicit per-registration target configuration while preserving source and freshness per field. |
