...
| Multi-writer behavior | two-worktree merge, rejected winner retry, reordered structured additions, conflict-identity role reversal, offline epoch reconnect, and out-of-tree conflict projection tests | complete |
...
| Conflict-storm scale | real installed-Git fixtures exercise 1,000 conflicts through complete paged/batched resolution in ordinary tests and 10,000 conflicts through bounded partial progress in scheduled hardening; both retain clean/structured results, immutable evidence, and the untouched normal index | complete on Linux; platform gates below remain |
