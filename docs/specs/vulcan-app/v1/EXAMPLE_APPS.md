...
For vault-owned scheduled ingestion, subscriptions, their stable identities, and capture rules are canonical portable records. Private subscriptions remain available for explicitly local/manual use. Authoritative capture bindings use the portable external-document binding model outside `cache.db`, so another assigned node can recognize previously imported articles; never keep their only copy in a private device database or add implementation-only sync markers to note frontmatter. A `device-local` typed/SQLite store holds read/star/archive state and optional binding projections; a `derived-cache` store holds entries, revisions, bodies, and refresh optimizations such as ETags. Cache eviction cannot remove canonical binding authority. Journal content/binding updates as recoverable steps before advancing import checkpoints, without claiming a cross-store SQLite transaction. Enclosures use the blob store. Saved entries become ordinary Markdown with `type: feed-entry`, stable source/feed IDs, source URL, published/fetched timestamps, content digest, and user-facing capture provenance. Remote disappearance never deletes a saved note. Default capture is import-once; source refresh requires explicit policy preserving user edits.

Scheduling follows [the shared execution contract](../../scheduled-execution.md): canonical enabled state and target node assignment, daemon live reload, optional trusted-commit verification, and no Feed Reader timer or local enable switch. Fetch/prepare outside mutation locks, then recheck source/binding/note revisions under the ordinary write workflow. Handoff synchronizes canonical notes and bindings; an offline old node may continue until it receives reassignment. Duplicate-aware import and preserved conflicts handle overlap without claiming distributed exclusion. Test handoff, offline overlap, interrupted binding/content writes, and signed capture-rule changes alongside the fixtures below.
